...
fn tier_fixture_classifies_c_m_q() {
    ...
    let tiers: Vec<(nlint::Tier, Option<String>)> = r.diagnostics.iter()
        .filter(|d| d.rule == "R2")
    ...
    let q_reason = tiers.iter().find(|(t, _)| *t == nlint::Tier::Q)
        .and_then(|(_, r)| r.as_ref())
        .expect("Q tier must have demotion_reason");
    assert!(q_reason.contains("Add") || q_reason.contains("arithmetic"),
        "Q demotion reason should name arithmetic; got: {}", q_reason);
}
...
fn r3_triggers_on_tuple_sealed_adds_key() {
    ...
    let r3_errors: Vec<_> = r.diagnostics.iter()
        .filter(|d| d.rule == "R3" && d.severity == nlint::Severity::Error)
        .collect();
    assert_eq!(r3_errors.len(), 1, "expected exactly 1 R3 error, got {:?}", r3_errors);
    assert!(r3_errors[0].msg.contains("#missing_key"), "R3 msg: {}", r3_errors[0].msg);
    assert!(r3_errors[0].msg.contains("s"), "R3 msg should name key s: {}", r3_errors[0].msg);
}
...
fn r3_does_not_trigger_on_open_combo_with_spread() {
    ...
    let r3_errors: Vec<_> = r.diagnostics.iter()
        .filter(|d| d.rule == "R3" && d.severity == nlint::Severity::Error)
        .collect();
    assert_eq!(r3_errors.len(), 1, "r3_safe must NOT trigger R3; got {:?}", r3_errors);
}
...
fn omega_fixture_reports_clique_4() {
    ...
    assert_eq!(r.graph.contexts.len(), 4, "expected 4 contexts, got {}", r.graph.contexts.len());
    ...
    assert!(w.shared_coords.iter().any(|c| c.contains("shared")),
        "K4 witness should share `shared` coord; got {:?}", w.shared_coords);
}
...
fn dollar_scan_boundary_outer_pipe_is_c() {
    ...
    let outer_tiers: Vec<_> = r.diagnostics.iter()
        .filter(|d| d.rule == "R2" && d.tier == Some(nlint::Tier::C))
        .collect();
    assert!(!outer_tiers.is_empty(), "outer pipe should be tier C (nested $ is scan-boundary); got {:?}",
        r.diagnostics.iter().filter(|d| d.rule == "R2").map(|d| d.tier).collect::<Vec<_>>());
}
...
fn all_unit_corpus_parses_or_skips_cleanly() {
    ...
    dir.push("../../tests/unit");
    if !dir.is_dir() { return; } // corpus may live elsewhere in workspaces
    ...
    for f in &files {
        ...
        if r.parse_error.is_some() { skip += 1; } else { ok += 1; }
    }
    // No hard count assertion — just that we ran without panic.
    eprintln!("unit corpus: {} parsed, {} skipped (parse failures)", ok, skip);
}
...
fn collect_n(dir: &std::path::Path, out: &mut Vec<PathBuf>) {
    if let Ok(entries) = std::fs::read_dir(dir) {
        for e in entries.flatten() {
            let p = e.path();
            if p.is_dir() { collect_n(&p, out); }
            else if p.extension().and_then(|s| s.to_str()) == Some("n") { out.push(p); }
        }
    }
}
...
fn free_dollar_scan_respects_pipe_boundary() {
    ...
    assert!(!nlint::has_free_dollar(&e), "nested pipe RHS $ must be scan-boundary");
}
...
fn free_dollar_scan_respects_morphism_body() {
    ...
    let src = "(x -> $)";
    let e = nlang_parser::parse_expr_only(src).unwrap();
    assert!(!nlint::has_free_dollar(&e), "morphism body $ is bound, not free");
}
...
fn r3_does_not_fire_for_transformer_with_spread() {
    ...
    let r = nlint::check_r3(&lhs, &rhs);
    assert!(r.is_none(), "R3 should NOT fire when transformer has spread (keys uncertain)");
}
...
fn atom_form_is_tier_c_rerun_safe() {
    ...
    assert!(diags.iter().any(|d| d.rule == "R1" && d.tier == Some(nlint::Tier::C)),
        "atom-form pipe must be marked rerun-safe, got {:?}", diags);
}

#[test]
fn unknown_form_emits_tier_u() {
    let diags = lint_snippet("unknown", "r: data |> handler");
    assert!(diags.iter().any(|d| d.rule == "R2" && d.tier == Some(nlint::Tier::U)),
        "unknown-form pipe must emit tier U, got {:?}", diags);
}

#[test]
fn morphism_form_stays_silent() {
    let diags = lint_snippet("morph", "r: 1 |> (x -> $ + 1)");
    assert!(diags.iter().all(|d| d.rule != "R1" && d.rule != "R2"),
        "morphism-form pipe gets no refinement-tier diagnostics, got {:?}", diags);
}
