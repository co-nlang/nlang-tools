...
fn test_env_get_existing() {
    ...
    let r = call(
        ...
        &mut ctx,
        "env.get",
        combo1(str_val("NLANG_TEST_VAR_P36")),
    );
    assert!(matches!(
        &r,
        Value::Atom(AtomKind::Str(_), EffectTag::IO, _)
    ));
...
fn test_env_get_nonexistent_returns_none() {
    ...
    let r = call(
        ...
        &mut ctx,
        "env.get",
        combo1(str_val("NLANG_DEFINITELY_NOT_SET_ABCXYZ123")),
    );
    assert!(is_none(&r));
...
fn test_env_args_returns_list() {
    ...
    if let Value::Combo(ref c) = r {
        assert!(
            c.get_field("0").is_some(),
            "env.args must return at least argv[0]"
        );
        assert!(matches!(
            c.get_field("0").unwrap(),
            Value::Atom(AtomKind::Str(_), EffectTag::IO, _)
        ));
    }
}
