use nlang_interpreter::{Ouroboros, Value, ComboVal, EffectTag, EvalContext};
...
fn test_morphism_tainting() {
    ...
    // 應用 IO 態射 -> 結果應帶 IO 顏色
    let res = oo.apply_morphism(now_m, Value::Atom(AtomKind::Unit, EffectTag::Pure, None), &mut ctx);
