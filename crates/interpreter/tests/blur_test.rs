...
use nlang_interpreter::value::{Value, BlurDetail, BlurCause, HorizonParams, EffectTag, ContentHash, ComboVal, ObservationStrategy};
...
fn make_blur(fuel: u64, cause: BlurCause, salt: ContentHash) -> Value {
    Value::Blur(BlurDetail {
        cause,
        horizon: HorizonParams { fuel_remaining: fuel, strategy: ObservationStrategy::Blur, salt },
        partial: None,
...
fn blur_fuel_caid_deterministic() {
    let bd1 = BlurDetail { cause: BlurCause::FuelExhausted, horizon: HorizonParams { fuel_remaining: 42, strategy: ObservationStrategy::Blur, salt: salt0() }, partial: None, effect: EffectTag::Pure };
    let bd2 = bd1.clone();
...
fn blur_different_fuel_different_caid() {
    let make = |fuel: u64| BlurDetail { cause: BlurCause::FuelExhausted, horizon: HorizonParams { fuel_remaining: fuel, strategy: ObservationStrategy::Blur, salt: salt0() }, partial: None, effect: EffectTag::Pure };
...
fn math_ln_zero_strict_returns_blur() {
    let oo = Ouroboros::new_in_memory();
    let mut ctx = EvalContext::new(oo.root_with_system()).with_strategy(ObservationStrategy::Strict);
    let root = oo.root_with_system();
    let math_combo = root.get_field("~%Math").unwrap().clone();
    let ln_morph = match &math_combo {
        Value::Combo(c) => c.get_field("/ln").unwrap().clone(),
...
fn blur_content_hash_deterministic() {
    let bd = BlurDetail { cause: BlurCause::MathSingularity("log_singularity".to_string()), horizon: HorizonParams { fuel_remaining: 100, strategy: ObservationStrategy::Blur, salt: salt1() }, partial: None, effect: EffectTag::Pure };
    let v = Value::Blur(bd);
...
fn blur_bn_serial_deterministic() {
    ...
    let bd = BlurDetail { cause: BlurCause::FuelExhausted, horizon: HorizonParams { fuel_remaining: 0, strategy: ObservationStrategy::Blur, salt: salt0() }, partial: None, effect: EffectTag::Pure };
    let v = Value::Blur(bd);
