...
fn test_list_any_true() {
    ...
    assert!(
        is_tag(&result, "true"),
        "list.any with closed combo should be #true: {:?}",
        result
    );
}
...
fn test_list_any_false() {
    ...
    assert!(
        is_tag(&result, "false"),
        "list.any with no closed combo should be #false: {:?}",
        result
    );
}
...
fn test_list_any_empty() {
    ...
    assert!(
        is_tag(&result, "false"),
        "list.any empty should be #false: {:?}",
        result
    );
}
...
fn test_list_all_true() {
    ...
    assert!(
        is_tag(&result, "true"),
        "list.all all closed should be #true: {:?}",
        result
    );
}
...
fn test_list_all_false() {
    ...
    assert!(
        is_tag(&result, "false"),
        "list.all with open combo should be #false: {:?}",
        result
    );
}
...
fn test_list_find_found() {
    ...
    if let Value::Combo(ref cv) = result {
        assert!(
            cv.get_field("%val").is_some(),
            "list.find closed should return Some: {:?}",
            result
        );
    } else {
...
fn test_list_find_not_found() {
    ...
    assert!(
        is_tag(&result, "none"),
        "list.find with no cocoon should be #none: {:?}",
        result
    );
}
