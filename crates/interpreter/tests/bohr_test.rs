...
use nlang_interpreter::value::{
    CaidVersion, ComboVal, ContentHash, EffectTag, HashAlgorithm, MasaRef, Value,
};
...
fn test_project_down_adds_blur_tag() {
    ...
    let arg_val = Value::Combo(ComboVal::new(
    ...
    assert!(
        matches!(result, Value::Combo(_)),
        "project_down should return a Combo"
    );
}
...
fn test_project_down_noncombo_target() {
    ...
    assert!(
        matches!(result, Value::Combo(_)),
        "should return a Combo wrapping the atom"
    );
}
...
fn test_project_up_single_section() {
    let (oo, mut ctx) = setup_bohr();
    let section = Value::Combo(ComboVal::new(
        IndexMap::from_iter(vec![(
            "x".to_string(),
    ...
    let sections = Value::Combo(ComboVal::new(
        IndexMap::from_iter(vec![
            ("0".to_string(), section),
            (
                "%kind".to_string(),
    ...
    let arg_val = Value::Combo(ComboVal::new(
        IndexMap::from_iter(vec![("sections".to_string(), sections)]),
        false,
        IndexMap::new(),
        EffectTag::Pure,
        vec![],
    ...
    assert!(
        ...
        "single section should not produce Bottom"
    );
}
...
fn test_project_down_top_masa() {
    ...
    let target = Value::Combo(ComboVal::new(
        data,
        false,
        IndexMap::new(),
        EffectTag::Pure,
    ...
    assert!(
        matches!(result, Value::Combo(_)),
        "Top MASA project_down should return Combo"
    );
}
...
fn test_project_up_compatible_sections() {
    ...
    assert!(
        !matches!(result, Value::Bottom(_)),
        "compatible sections should merge"
    );
}
