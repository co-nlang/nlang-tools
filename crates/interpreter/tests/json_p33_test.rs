...
fn list_str_at(v: &Value, i: usize) -> String {
    match v {
        Value::Combo(c) => as_str(c.get_field(&i.to_string()).unwrap()).to_string(),
        _ => panic!(),
    }
}
...
fn test_json_parse_object() {
    ...
    let r = call(
        ...
        &mut ctx,
        "json.parse",
        combo1(str_val(r#"{"name":"Alice","age":30}"#)),
    );
...
fn test_json_parse_primitives() {
    ...
    assert!(
        matches!(r_true,  Value::Atom(AtomKind::Tag(t), _, _) if t.trim_start_matches('#') == "true")
    );
    assert!(
        matches!(r_false, Value::Atom(AtomKind::Tag(t), _, _) if t.trim_start_matches('#') == "false")
    );
}
...
fn test_json_stringify_list() {
    ...
    let list = Value::Combo(ComboVal::new(
        m,
        false,
        IndexMap::new(),
        EffectTag::Pure,
