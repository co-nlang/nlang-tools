...
fn test_url_parse_components() {
    ...
    let r = call(
        ...
        &mut ctx,
        "url.parse",
        str_val("https://example.com/path?key=val#frag"),
    );
    if let Value::Combo(c) = &r {
        assert!(
            matches!(c.get_field("scheme"), Some(Value::Atom(AtomKind::Str(s), _, _)) if s == "https")
        );
        assert!(
            matches!(c.get_field("host"),   Some(Value::Atom(AtomKind::Str(s), _, _)) if s == "example.com")
        );
        assert!(
            matches!(c.get_field("path"),   Some(Value::Atom(AtomKind::Str(s), _, _)) if s == "/path")
        );
    } else {
...
fn test_url_query_params() {
    ...
    let r = call(
        ...
        &mut ctx,
        "url.query_params",
        str_val("https://x.com/?foo=1&bar=2"),
    );
    if let Value::Combo(c) = &r {
        assert!(
            matches!(c.get_field("foo"), Some(Value::Atom(AtomKind::Str(s), _, _)) if s == "1")
        );
        assert!(
            matches!(c.get_field("bar"), Some(Value::Atom(AtomKind::Str(s), _, _)) if s == "2")
        );
    } else {
        panic!("expected Combo");
    }
