...
fn elems(v: &Value) -> Vec<Value> {
    match v {
        Value::Combo(cv) => {
            ...
            keys.sort_unstable();
            keys.into_iter().map(|k| cv.data.get(&k.to_string()).cloned().unwrap()).collect()
        }
...
fn tuple_splice_stays_sealed() {
    let v = eval_one("r: (...(1, 2), 3)");
    assert_eq!(elems(&v), vec![int(1), int(2), int(3)]);
    match v { Value::Combo(cv) => assert!(cv.closed, "tuple result must stay sealed"), _ => unreachable!() }
}
...
fn tuple_splices_into_list() {
    ...
    let v = eval_one("r: [...(1, 2), 3]");
    assert_eq!(elems(&v), vec![int(1), int(2), int(3)]);
    match v { Value::Combo(cv) => assert!(!cv.closed), _ => unreachable!() }
}
...
fn bottom_source_collapses_container() {
    ...
    assert!(is_bottom, "bottom spread source must collapse the list, got {:?}", v);
}
