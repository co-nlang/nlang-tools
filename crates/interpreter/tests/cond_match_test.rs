...
fn match_no_pattern_returns_not_bottom() {
    ...
    assert!(!matches!(result.collapse(), Value::Bottom(_)),
        "No match should not be Bottom: {:?}", result);
    // The result should not equal the test value either (pattern didn't match)
    assert_ne!(result.collapse().to_string_plain(), "#qux",
