...
fn test_select_nested_path() {
    ...
    let result = call(
        ...
        &mut ctx,
        "query.select",
        args2(outer, str_val("address.city")),
    );
...
fn test_select_list_index() {
    ...
    let result = call(
        ...
        &mut ctx,
        "query.select",
        args2(container, str_val("items.1")),
    );
...
fn test_pluck_extracts_specified_fields() {
    ...
    if let Value::Combo(ref cv) = result {
        ...
        panic!("expected Combo, got {:?}", result);
    }
}
...
fn test_where_empty_list_returns_empty() {
    ...
    if let Value::Combo(ref cv) = result {
        assert!(
            ...
            "empty list should have no items"
        );
    } else {
        panic!("expected Combo list, got {:?}", result);
    }
}
