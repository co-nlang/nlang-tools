...
use nlang_interpreter::value::{
    default_cache_id, Commit, CommitKind, CommitMeta, ContentHash, RefineInfo,
};
...
fn refine_simple_source_to_target() {
    ...
    let val_b = Value::Atom(
        nlang_parser::ast::AtomKind::Int(5.into()),
        EffectTag::Pure,
        None,
    );
    ...
    assert_ne!(
        caid_a, caid_b,
        "A and B must have different CAIDs for refine test"
    );
...
fn refine_fails_monotonicity() {
    ...
    let val_a = Value::Atom(
        nlang_parser::ast::AtomKind::Int(1.into()),
        EffectTag::Pure,
        None,
    );
    let caid_a = oo.store.put_value(&val_a).unwrap();
    let val_b = Value::Atom(
        nlang_parser::ast::AtomKind::Int(2.into()),
        EffectTag::Pure,
        None,
    );
    let caid_b = oo.store.put_value(&val_b).unwrap();
...
fn refine_cycle_detection() {
    ...
    let v1 = Value::Top;
    let v2 = Value::Atom(
        nlang_parser::ast::AtomKind::Int(42.into()),
        EffectTag::Pure,
        None,
    );
...
fn refine_max_hops() {
    ...
    for i in 0u8..18 {
        values.push(Value::Atom(
            nlang_parser::ast::AtomKind::Int(i.into()),
            EffectTag::Pure,
            None,
        ));
    }
    ...
    let caids: Vec<ContentHash> = values
        .iter()
        .map(|v| oo.store.put_value(v).unwrap())
        .collect();
    ...
    // Manually build the chain: each Ai → Ai+1
    {
        let mut map = oo.refine_map.write().unwrap();
        for i in 0..17 {
            map.entry(caids[i].to_string())
                .or_default()
                .push(caids[i + 1].to_string());
        }
    }
...
fn refine_no_redirect_in_history_commits() {
    ...
    assert_eq!(
        direct_a, val_a,
        "history get_value should return original, not redirected"
    );
}
...
fn refine_info_stored_in_commit() {
    ...
    let src = Value::Top;
    let tgt = Value::Atom(
        nlang_parser::ast::AtomKind::Int(7.into()),
        EffectTag::Pure,
        None,
    );
    ...
    assert_eq!(
        commit.kind,
        CommitKind::Refine,
        "commit kind should be Refine"
    );
    assert!(
        commit.refine_info.is_some(),
        "refine_info should be present"
    );
...
fn get_live_value_follows_refine() {
    ...
    let v1 = Value::Top;
    let v2 = Value::Atom(
        nlang_parser::ast::AtomKind::Int(99.into()),
        EffectTag::Pure,
        None,
    );
    let caid1 = oo.store.put_value(&v1).unwrap();
...
fn shadow_affected_empty_on_fresh_universe() {
    ...
    let ri = commit.refine_info.unwrap();
    assert!(
        ri.shadow_affected.is_empty(),
        "fresh universe → no shadow history"
    );
}
...
fn shadow_affected_detects_historical_usage() {
    ...
    assert!(
        ri2.shadow_affected.len() <= 16,
        "shadow scan bounded to 16 commits"
    );
}
...
fn refine_cycle_ab_ba_rejected() {
    ...
    assert!(
        result.is_err(),
        "A→B with A→B→A cycle should be rejected: {:?}",
        result
    );
