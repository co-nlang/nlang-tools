...
fn test_str_parse_int_ok() {
    ...
    match r {
        Value::Atom(AtomKind::Int(n), _, _) => assert_eq!(n, BigInt::from(42)),
...
fn test_str_parse_int_negative() {
    ...
    match r {
        Value::Atom(AtomKind::Int(n), _, _) => assert_eq!(n, BigInt::from(-7)),
        _ => panic!("expected Int"),
    }
}
...
fn test_str_parse_int_invalid() {
    ...
    assert!(
        matches!(r, Value::Bottom(_)),
        "expected Bottom on invalid parse"
    );
}
...
fn test_str_from_int() {
    ...
    match r {
        Value::Atom(AtomKind::Str(s), _, _) => assert_eq!(s, "99"),
...
fn test_str_from_int_float() {
    ...
    match r {
        Value::Atom(AtomKind::Str(s), _, _) => assert!(s.contains("3.14"), "got: {}", s),
        _ => panic!("expected Str"),
    }
}
...
fn test_str_repeat() {
    ...
    let r = call(
        ...
        &mut ctx,
        "str.repeat",
        make_combo_2(int_val(3), str_val("ab")),
    );
    match r {
        Value::Atom(AtomKind::Str(s), _, _) => assert_eq!(s, "ababab"),
        _ => panic!("expected Str"),
    }
}
