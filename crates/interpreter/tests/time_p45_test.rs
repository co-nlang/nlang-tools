...
fn test_time_parse_and_to_iso8601() {
    ...
    let r = call(
        ...
        &mut ctx,
        "time.parse",
        combo2(str_val("2023-01-15 10:30:00"), str_val("%Y-%m-%d %H:%M:%S")),
    );
    let ts = as_i64(&r);
    ...
    let s = as_str(&r2);
    assert!(
        s.starts_with("2023-01-15T10:30:00"),
        "to_iso8601 should start with 2023-01-15T10:30:00, got {}",
        s
    );
}
