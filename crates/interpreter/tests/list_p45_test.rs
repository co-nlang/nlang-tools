...
fn float(n: f64) -> Value {
    Value::Atom(AtomKind::Float(n), EffectTag::Pure, None)
}
...
fn list_at(v: &Value, i: usize) -> &Value {
    match v {
        Value::Combo(c) => c.get_field(&i.to_string()).unwrap(),
        _ => panic!(),
    }
}
    ...
    Value::Combo(ComboVal::new(
        IndexMap::from_iter(vec![
            ...
            (
                "%builtin".to_string(),
                Value::Atom(
                    AtomKind::Str("math.is_prime".to_string()),
                    EffectTag::Pure,
                    None,
                ),
            ),
...
fn test_list_product() {
    ...
    let r = call(
        ...
        &mut ctx,
        "list.product",
        combo1(list(vec![int(2), int(3), int(4)])),
    );
    ...
    let r = call(
        ...
        &mut ctx,
        "list.product",
        combo1(list(vec![float(2.5), int(2)])),
    );
