...
fn at_option_in_root_with_system() {
    ...
    let root = oo.root_with_system();
    assert!(
        root.get_field("@option").is_some(),
        "@option should be in root_with_system"
    );
}
...
fn at_result_in_root_with_system() {
    ...
    let root = oo.root_with_system();
    assert!(
        root.get_field("@result").is_some(),
        "@result should be in root_with_system"
    );
}
...
fn type_constraint_option_accepts_none() {
    ...
    assert!(matches!(
        TypeConstraint::Option.validate_value(&v),
...
fn type_constraint_result_accepts_ok_and_err() {
    ...
    let ok = Value::Combo(ComboVal::new(
        ok_fields,
        false,
        IndexMap::new(),
        EffectTag::Pure,
    ...
    assert!(matches!(
        TypeConstraint::Result.validate_value(&ok),
        ValidationResult::Pass
    ));
    ...
    err_fields.insert(
        "%cause".to_string(),
        Value::Atom(AtomKind::Tag("timeout".to_string()), EffectTag::Pure, None),
    );
    let err = Value::Combo(ComboVal::new(
        err_fields,
        false,
        IndexMap::new(),
        EffectTag::Pure,
    ...
    assert!(matches!(
        TypeConstraint::Result.validate_value(&err),
        ValidationResult::Pass
    ));
}
...
fn eval_context_reads_config_fuel() {
    ...
    assert_eq!(
        ctx.fuel, 10000,
        "eval_context() should read fuel from ~%Config"
    );
}
...
fn eval_context_reads_config_max_branches() {
    ...
    assert_eq!(
        ctx.max_branches, 64,
        "eval_context() should read max_branches from ~%Config"
    );
}
...
fn eval_context_reads_config_strategy() {
    ...
    let ctx = oo.eval_context();
    assert!(
        matches!(ctx.strategy, ObservationStrategy::Blur),
