...
use nlang_interpreter::{ComboVal, EffectTag, Value};
...
fn test_private_field_caid() {
    // { public: 1, ~secret: 2 }
    let mut f1 = IndexMap::new();
    f1.insert("public".to_string(), int_atom(1));
    let mut l1 = IndexMap::new();
    l1.insert("secret".to_string(), int_atom(2));
    let c1 = Value::Combo(ComboVal::new(f1, false, l1, EffectTag::Pure, vec![]));

    // { public: 1 } (無私有欄位)
    let mut f2 = IndexMap::new();
    f2.insert("public".to_string(), int_atom(1));
    let c2 = Value::Combo(ComboVal::new(
        f2,
        false,
        IndexMap::new(),
