...
impl Ouroboros {
    pub fn dispatch_morphism(&self, rules: &ComboVal, arg: &Value, ctx: &mut EvalContext) -> MorphismDispatchResult {
        let mut matching_branches: Vec<(String, Value, Value)> = Vec::new();
        
for (pattern_key, rule_val) in rules.all_fields_iter() {
            if pattern_key.starts_with('%') { continue; }
            ...
            if !matches!(unified, Value::Bottom(_)) {
                matching_branches.push((pattern_key.clone(), unified.clone(), rule_val.clone()));
            }
        }
        ...
        match minimal_elements.len() {
            ...
            1 => {
                let (pattern_key, _, rule) = &minimal_elements[0];
                let result = self.apply_single_rule(rule.clone(), arg.clone(), pattern_key.clone(), ctx);
                MorphismDispatchResult::Single(result)
            }
            _ => {
                let results: Vec<Value> = minimal_elements.iter().map(|(pattern_key, _, rule)| {
                    self.apply_single_rule(rule.clone(), arg.clone(), pattern_key.clone(), ctx)
                }).filter(|v| !matches!(v, Value::Bottom(_))).collect();
    ...
    fn resolve_pattern(&self, pattern_key: &str, _ctx: &mut EvalContext) -> Value {
        ...
        if trimmed.parse::<BigInt>().is_ok() {
            return Value::Atom(AtomKind::Int(trimmed.parse::<BigInt>().unwrap()), EffectTag::Pure, None);
        }
        
        if trimmed.parse::<f64>().is_ok() {
            return Value::Atom(AtomKind::Float(trimmed.parse::<f64>().unwrap()), EffectTag::Pure, None);
        }
    ...
    fn filter_minimal_branches(&self, branches: &[(String, Value, Value)], ctx: &mut EvalContext) -> Vec<(String, Value, Value)> {
        let n = branches.len();
        if n <= 1 {
        ...
        for i in 0..n {
            for j in 0..n {
                ...
                let (_, unified_i, _) = &branches[i];
                let (_, unified_j, _) = &branches[j];
                
                let meet_ij = self.unify_internal(unified_i.clone(), unified_j.clone(), ctx);
                
                if meet_ij == *unified_j && meet_ij != *unified_i {
                    is_minimal[j] = false;
                }
            }
        }
    ...
    fn apply_single_rule(&self, rule: Value, arg: Value, pattern_key: String, ctx: &mut EvalContext) -> Value {
        ...
        if let Value::Combo(ref rc) = rule {
            if let Some(Value::Code(expr)) = rc.get_field("%code") {
                ...
                call_ctx.scopes.push(ComboVal::new(arg_map, false, IndexMap::new(), EffectTag::Pure, vec![]));
