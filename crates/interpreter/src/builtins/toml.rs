...
pub fn register_toml_builtins(m: &mut HashMap<String, Arc<BuiltinFn>>) {
    m.insert("toml.parse".to_string(), Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
    ...
    m.insert("toml.stringify".to_string(), Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
        let v = oo.force(arg, ctx);
        let target = match &v {
            Value::Combo(c) => match c.get_field("0") {
