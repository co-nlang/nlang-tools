...
pub fn register_cond_builtins(m: &mut HashMap<String, Arc<BuiltinFn>>) {
    m.insert("cond.if".to_string(), Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
        if let Value::Combo(ref c) = arg {
            if let (Some(cond), Some(then_b), Some(else_b)) = (c.get_field("0"), c.get_field("1"), c.get_field("2")) {
                let fc = oo.force(cond.clone(), ctx);
                if let Value::Atom(AtomKind::Tag(ref t), ce, _) = fc.collapse() {
                    let branch = if t.trim_start_matches('#') == "true" { then_b } else { else_b };
                    let res = oo.apply_morphism(branch.clone(), Value::Atom(AtomKind::Unit, EffectTag::Pure, None), ctx);
    ...
    m.insert("cond.cond".to_string(), Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
                ...
                for (_k, pair_v) in &cv.fields() {
                    if _k.parse::<usize>().is_ok() {
                        ...
                        if let Value::Combo(ref pc) = fpair.collapse() {
                            if let (Some(cond_m), Some(action_m)) = (pc.get_field("0"), pc.get_field("1")) {
                                let fc = oo.force(cond_m.clone(), ctx);
                                if let Value::Atom(AtomKind::Tag(ref t), ce, _) = fc.collapse() {
                                    if t.trim_start_matches('#') == "true" {
                                        let res = oo.apply_morphism(action_m.clone(), Value::Atom(AtomKind::Unit, EffectTag::Pure, None), ctx);
                                        return res.with_effect(*ce);
                                    }
                                }
                            }
