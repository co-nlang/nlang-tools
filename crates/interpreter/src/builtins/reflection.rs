...
pub fn register_reflection_builtins(m: &mut HashMap<String, Arc<BuiltinFn>>) {
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            if let Value::Combo(c) = oo.force(v, ctx).collapse() {
                ...
                for k in keys {
                    res.insert(
                        count.to_string(),
                        Value::Atom(AtomKind::Str(k), EffectTag::Pure, None),
                    );
                    count += 1;
                }
                res.insert(
            ...
            if let Value::Combo(ref c) = arg {
                if let (Some(vkey), Some(vobj)) = (c.get_field("0"), c.get_field("1")) {
                    ...
                    if let Value::Combo(oc) = oo.force(vobj.clone(), ctx).collapse() {
                        return Value::Atom(
                            AtomKind::Tag(if oc.fields().contains_key(&key) {
                                "true".to_string()
                            } else {
                                "false".to_string()
                            }),
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            Value::Atom(
                AtomKind::Tag(if is { "true" } else { "false" }.to_string()),
                EffectTag::Pure,
                None,
            )
        }) as Arc<BuiltinFn>,
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            let fv = oo.force(v, ctx);
            Value::Atom(
                AtomKind::Str(fv.collapse().to_string_plain()),
                EffectTag::Pure,
                None,
            )
        }) as Arc<BuiltinFn>,
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            if let Value::Bottom(ref bd) = fv.collapse() {
                Value::Atom(
                    AtomKind::Tag(bd.cause.as_tag().to_string()),
                    EffectTag::Pure,
                    None,
                )
            } else {
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            if let Value::Combo(c) = oo.force(v, ctx).collapse() {
                let mut pairs: Vec<(String, Value)> = c
                    .fields()
                    .into_iter()
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            if let Value::Combo(c) = oo.force(v, ctx).collapse() {
                ...
                for (i, (key, val)) in pairs.into_iter().enumerate() {
                    ...
                    entry.insert(
                        "key".to_string(),
                        Value::Atom(AtomKind::Str(key), EffectTag::Pure, None),
                    );
                    entry.insert("val".to_string(), val);
                    let entry_combo =
                        ComboVal::new(entry, true, IndexMap::new(), EffectTag::Pure, vec![]);
