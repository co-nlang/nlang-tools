...
pub fn register_path_builtins(m: &mut HashMap<String, Arc<BuiltinFn>>) {
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            if let Value::Combo(ref c) = arg {
                if let (Some(v0), Some(v1)) = (c.get_field("0"), c.get_field("1")) {
                    ...
                    if let (
                        Value::Atom(AtomKind::Str(base), _, _),
                        Value::Atom(AtomKind::Str(seg), _, _),
                    ) = (f0.collapse(), f1.collapse())
                    {
                        let joined = Path::new(base.as_str()).join(seg.as_str());
                        return Value::Atom(
                            AtomKind::Str(joined.to_string_lossy().into_owned()),
                            EffectTag::Pure,
                            None,
                        );
                    }
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            if let Value::Atom(AtomKind::Str(path), _, _) = forced.collapse() {
                return match Path::new(path.as_str()).parent() {
                    Some(p) => Value::Atom(
                        AtomKind::Str(p.to_string_lossy().into_owned()),
                        EffectTag::Pure,
                        None,
                    ),
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            if let Value::Atom(AtomKind::Str(path), _, _) = forced.collapse() {
                return match Path::new(path.as_str()).file_name() {
                    Some(name) => Value::Atom(
                        AtomKind::Str(name.to_string_lossy().into_owned()),
                        EffectTag::Pure,
                        None,
                    ),
                    None => Value::Atom(AtomKind::Tag("none".to_string()), EffectTag::Pure, None),
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            if let Value::Atom(AtomKind::Str(path), _, _) = forced.collapse() {
                return match Path::new(path.as_str()).extension() {
                    Some(ext) => Value::Atom(
                        AtomKind::Str(ext.to_string_lossy().into_owned()),
                        EffectTag::Pure,
                        None,
                    ),
                    None => Value::Atom(AtomKind::Tag("none".to_string()), EffectTag::Pure, None),
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            let forced = oo.force(v, ctx);
            if let Value::Atom(AtomKind::Str(path), _, _) = forced.collapse() {
                let tag = if Path::new(path.as_str()).is_absolute() {
                    "true"
                } else {
                    "false"
                };
