...
pub fn register_stat_builtins(m: &mut HashMap<String, Arc<BuiltinFn>>) {
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            let nums = match extract_floats(&v, oo, ctx) {
                Some(n) => n,
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            let v = oo.force(arg, ctx);
            let mut nums = match extract_floats(&v, oo, ctx) {
                Some(n) => n,
                None => return conflict(),
            };
            if nums.is_empty() {
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            let mut nums = match extract_floats(&list_v, oo, ctx) {
                Some(n) => n,
                None => return conflict(),
            };
            if nums.is_empty() {
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            let nums = match extract_floats(&list_v, oo, ctx) {
                Some(n) => n,
                None => return conflict(),
            };
            if nums.is_empty() {
            ...
            let width = if (max - min).abs() < 1e-15 {
                1.0
            } else {
                (max - min) / bins as f64
            };
