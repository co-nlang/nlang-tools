...
use crate::value::{BottomCause, BottomDetail, ComboVal, ContentHash, EffectTag, MasaRef, Value};
...
pub fn register_engine_builtins(m: &mut HashMap<String, Arc<BuiltinFn>>) {
    ...
    m.insert(
        ...
        Arc::new(
            |arg: Value, _oo: &Ouroboros, _ctx: &mut EvalContext| match &arg {
                Value::Atom(AtomKind::Int(n), _, _) => {
                    let tag = match n.to_u8().unwrap_or(0) {
                        1 => "d1_converging",
                        2 => "d2_branching",
                        3 => "d3_horizon",
                        _ => "unknown",
                ...
                Value::Combo(ref c) => {
                    if let Some(Value::Atom(AtomKind::Int(d), _, _)) = c.get_field("%degree") {
                        let tag = match d.to_u8().unwrap_or(1) {
                            1 => "d1_converging",
                            2 => "d2_branching",
                            _ => "d3_horizon",
                        };
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            let sections: Vec<Value> = raw_sections
                .iter()
    ...
    m.insert(
        "engine.set_strategy".to_string(),
        Arc::new(|arg: Value, _oo: &Ouroboros, ctx: &mut EvalContext| {
            let tag = match &arg {
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            match result {
                crate::oml::OMLResult::Vacuous => Value::Atom(
                    AtomKind::Tag("oml_vacuous".to_string()),
                    EffectTag::Pure,
                    None,
                ),
                crate::oml::OMLResult::Valid => Value::Atom(
                    AtomKind::Tag("oml_valid".to_string()),
                    EffectTag::Pure,
                    None,
                ),
                crate::oml::OMLResult::Approximate => Value::Atom(
                    AtomKind::Tag("oml_approximate".to_string()),
                    EffectTag::Pure,
                    None,
                ),
                crate::oml::OMLResult::Violation { rhs, expected } => {
                    ...
                    fields.insert(
                        "%kind".to_string(),
                        Value::Atom(
                            AtomKind::Tag("oml_violation".to_string()),
                            EffectTag::Pure,
                            None,
                        ),
                    );
            ...
            if let Value::Combo(ref c) = arg {
                if let (Some(f), Some(opt_v)) = (c.get_field("0"), c.get_field("1")) {
                    ...
                    if let Some(val) = inner {
                        let mapped = if matches!(f, Value::Top) {
                            val
                        } else {
                            oo.apply_morphism(f, val, ctx)
                        };
    ...
    m.insert(
        "engine.equivalence_map".to_string(),
        Arc::new(|_arg: Value, oo: &Ouroboros, _ctx: &mut EvalContext| {
                ...
                if let Ok(from_hash) = ContentHash::parse(from_str) {
                    if let Ok(to_hash) = oo.follow_refine(&from_hash) {
                        ...
                        if to_str != *from_str {
                            ...
                            entry.insert(
                                "to".to_string(),
                                Value::Atom(AtomKind::Str(to_str), EffectTag::State, None),
                            );
                            entries.push(Value::Combo(ComboVal::new(
                                entry,
                                false,
                                IndexMap::new(),
                                EffectTag::State,
            ...
            list_fields.insert(
                "%kind".to_string(),
                Value::Atom(AtomKind::Tag("list".to_string()), EffectTag::State, None),
            );
            for (i, e) in entries.iter().enumerate() {
                list_fields.insert(i.to_string(), e.clone());
            }
            let entries_list = Value::Combo(ComboVal::new(
                list_fields,
                false,
                IndexMap::new(),
                EffectTag::State,
            ...
            result.insert(
                "%kind".to_string(),
                Value::Atom(
                    AtomKind::Tag("equivalence_map".to_string()),
                    EffectTag::Pure,
                    None,
                ),
            );
            result.insert(
                "%count".to_string(),
                Value::Atom(
                    AtomKind::Int(BigInt::from(entries.len() as i64)),
                    EffectTag::State,
                    None,
                ),
            );
            ...
            if let Value::Atom(AtomKind::Str(caid_str), _, _) = forced.collapse() {
                if let Ok(h) = ContentHash::parse(caid_str.as_str()) {
                    return match oo.follow_refine(&h) {
                        Ok(resolved) => {
                            Value::Atom(AtomKind::Str(resolved.to_string()), EffectTag::State, None)
                        }
                        Err(_) => Value::Top,
                    };
                }
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            if let Value::Combo(ref c) = arg {
                if let (Some(f), Some(opt_v)) = (c.get_field("0"), c.get_field("1")) {
                    ...
                    if let Some(val) = inner {
                        let applied = if matches!(f, Value::Top) {
                            val
                        } else {
                            oo.apply_morphism(f, val, ctx)
                        };
    ...
    m.insert(
        ...
        Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
            ...
            match &outer {
                ...
                Value::Combo(ref cv) => match cv.get_field("%val") {
                    None => Value::Top,
                    Some(inner) => {
                        ...
                        match &inner_forced {
                            ...
                            Value::Combo(ref icv) if icv.get_field("%val").is_some() => {
                                inner_forced.clone()
                            }
                            _ => Value::Top,
                        }
            ...
            match &outer {
                Value::Combo(ref cv) => {
                    ...
                    if let Some(inner) = cv.get_field("%val") {
                        ...
                        match &inner_forced {
                            Value::Combo(ref icv)
                                if icv.get_field("%val").is_some()
                                    || icv.get_field("%cause").is_some() =>
                            {
