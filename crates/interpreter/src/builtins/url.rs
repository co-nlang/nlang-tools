...
pub fn register_url_builtins(m: &mut HashMap<String, Arc<BuiltinFn>>) {
    m.insert("url.parse".to_string(), Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
    ...
    m.insert("url.encode".to_string(), Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
        let v = oo.force(arg, ctx);
        let s = match extract_str_arg(&v, oo, ctx) {
            Some(s) => s,
            None => return conflict(),
    ...
    m.insert("url.decode".to_string(), Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
        let v = oo.force(arg, ctx);
        let s = match extract_str_arg(&v, oo, ctx) {
            Some(s) => s,
            None => return conflict(),
    ...
    m.insert("url.join".to_string(), Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
    ...
    m.insert("url.query_params".to_string(), Arc::new(|arg: Value, oo: &Ouroboros, ctx: &mut EvalContext| {
        let v = oo.force(arg, ctx);
        let s = match extract_str_arg(&v, oo, ctx) {
            Some(s) => s,
            None => return conflict(),
