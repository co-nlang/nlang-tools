...
impl Program {
    ...
    pub fn strip_spans(&mut self) {
        for f in &mut self.fields { f.strip_spans(); }
    }
...
impl Expr {
    ...
    pub fn strip_spans(&mut self) {
        ...
        match &mut self.kind {
            ...
            ExprKind::Apply(a, b) | ExprKind::Pipe(a, b) | ExprKind::Meet(a, b) | ExprKind::Join(a, b) | ExprKind::Diff(a, b) |
            ExprKind::Add(a, b) | ExprKind::Sub(a, b) | ExprKind::Mul(a, b) | ExprKind::Div(a, b) | ExprKind::Rem(a, b) |
            ExprKind::Eq(a, b) | ExprKind::Ne(a, b) | ExprKind::Lt(a, b) | ExprKind::Gt(a, b) | ExprKind::Lte(a, b) | ExprKind::Gte(a, b) |
            ExprKind::TypeAnnotation(a, b) | ExprKind::Lens(a, b) | ExprKind::LatticeEq(a, b) | ExprKind::Probe(a, b) => {
                a.strip_spans(); b.strip_spans();
            }
            ExprKind::Morphism { param, body } => { param.strip_spans(); body.strip_spans(); }
            ExprKind::Combo { fields, relations, .. } => {
                for f in fields { f.strip_spans(); }
                for r in relations { r.strip_spans(); }
            }
            ExprKind::Ternary { cond, then_branch, else_branch } => {
                cond.strip_spans(); then_branch.strip_spans(); else_branch.strip_spans();
            }
            ExprKind::Unary { expr, .. } | ExprKind::AnonSet(expr) | ExprKind::Spread(expr) |
            ExprKind::Structural(expr) | ExprKind::Complement(expr) => expr.strip_spans(),
            ExprKind::List(items) | ExprKind::Tuple(items) => {
                for i in items { i.strip_spans(); }
            }
            ...
            ExprKind::Interpolated(parts) => {
                for part in parts {
                    if let StringPart::Interpolated(e) = part { e.strip_spans(); }
                }
            }
            ExprKind::Range { start, end, step } => {
                start.strip_spans(); end.strip_spans();
                if let Some(s) = step { s.strip_spans(); }
            }
        }
    }
    pub fn without_spans(&self) -> Self {
    ...
    pub fn shape(&self) -> String {
        match &self.kind {
            ...
            ExprKind::Morphism { param, body } => format!("Morphism({}, {})", param.shape(), body.shape()),
            ExprKind::Combo { fields, relations, closed } => {
                ...
                format!("Combo(closed={closed}, [{}], [{}])", fs.join(", "), rs.join(", "))
            }
            ...
            ExprKind::Ternary { cond, then_branch, else_branch } => {
                format!("Ternary({}, {}, {})", cond.shape(), then_branch.shape(), else_branch.shape())
            }
            ...
            ExprKind::TypeAnnotation(v, t) => format!("TypeAnnotation({}, {})", v.shape(), t.shape()),
            ...
            ExprKind::Interpolated(parts) => {
                let ps: Vec<_> = parts.iter().map(|p| match p {
    ...
    pub fn precedence(&self) -> u8 {
        match &self.kind {
            ...
            ExprKind::Eq(..) | ExprKind::Ne(..) | ExprKind::Lt(..) | ExprKind::Gt(..)
            | ExprKind::Lte(..) | ExprKind::Gte(..) | ExprKind::LatticeEq(..) | ExprKind::Probe(..) => 5,
            ExprKind::Meet(..) => 6,
...
impl Relation {
    pub fn strip_spans(&mut self) { self.span = Span::default(); }
    pub fn shape(&self) -> String {
        let os = match self.op {
            RelOp::Lt => "<", RelOp::Gt => ">", RelOp::Lte => "<=", RelOp::Gte => ">=", RelOp::Eq => "=",
        };
    ...
    pub fn to_string_canonical(&self) -> String {
        match self {
            ...
            AtomKind::Complex(r, i) => {
                ...
                if *r == 0.0 {
                    if *i == 1.0 { return "i".into(); }
                    if *i == -1.0 { return "-i".into(); }
                    if *i >= 0.0 { return format!("{i}i"); }
                    return format!("{i}i"); // already signed, e.g. -3i
                }
                if *i >= 0.0 { format!("{r}+{i}i") } else { format!("{r}{i}i") } // i already signed
            },
