...
use crate::ast::{Expr, ExprKind, Field, FieldKey, Path, PathAnchor, StringPart, UnaryOp};
...
fn has_free_dollar_inner(expr: &Expr, in_interp: bool) -> bool {
    match &expr.kind {
        ...
        ExprKind::Interpolated(parts) => {
            for part in parts {
                if let StringPart::Interpolated(e) = part {
                    if has_free_dollar_inner(e, true) {
                        return true;
                    }
                }
            }
        ...
        ExprKind::Combo { fields, .. } => {
            for f in fields {
                if has_free_dollar_field(f, in_interp) {
                    return true;
                }
            }
            false
        ...
        ExprKind::Apply(f, a) => {
            has_free_dollar_inner(f, in_interp) || has_free_dollar_inner(a, in_interp)
        }
        ExprKind::Meet(a, b)
        | ExprKind::Join(a, b)
        | ExprKind::Diff(a, b)
        ...
        ExprKind::Range { start, end, step } => {
            has_free_dollar_inner(start, in_interp)
                ...
                || step
                    .as_ref()
                    .map_or(false, |s| has_free_dollar_inner(s, in_interp))
        }
...
pub fn classify_tier(rhs: &Expr) -> (Tier, Option<String>) {
    ...
    match first_non_whitelist(rhs) {
        Some((desc, span)) => (
            Tier::Q,
            Some(format!("{} at span {}..{}", desc, span.0, span.1)),
        ),
        None => (Tier::M, None),
    }
}
...
fn first_non_whitelist(expr: &Expr) -> Option<(&'static str, (usize, usize))> {
    match &expr.kind {
        ...
        ExprKind::Combo { fields, .. } => {
            for f in fields {
                if let Some(r) = first_non_whitelist_field(f) {
                    return Some(r);
                }
            }
            None
        ...
        ExprKind::List(items) | ExprKind::Tuple(items) => {
            for i in items {
                if let Some(r) = first_non_whitelist(i) {
                    return Some(r);
                }
            }
            None
        ...
        ExprKind::Apply(_, _) => Some((
            "Apply (morphism application)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::Morphism { .. } => {
            Some(("Morphism literal `->`", (expr.span.start, expr.span.end)))
        }
        ExprKind::Ternary { .. } => Some(("Ternary `? :`", (expr.span.start, expr.span.end))),
        ExprKind::Diff(_, _) => Some((
            "Diff `\\` (not in M whitelist: only & and |)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::Complement(_) => Some((
            "Complement `!` (anti-monotone)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::Unary { op, .. } => Some((
            match op {
                UnaryOp::Not => "Unary `!`",
                UnaryOp::Neg => "Unary `-` (arithmetic)",
            },
            (expr.span.start, expr.span.end),
        )),
        ...
        ExprKind::Eq(_, _) => Some((
            "Eq `==` (booleanizing comparison)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::Ne(_, _) => Some((
            "Ne `!=` (booleanizing comparison)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::Lt(_, _) => Some((
            "Lt `<` (booleanizing comparison)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::Gt(_, _) => Some((
            "Gt `>` (booleanizing comparison)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::Lte(_, _) => Some((
            "Lte `<=` (booleanizing comparison)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::Gte(_, _) => Some((
            "Gte `>=` (booleanizing comparison)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::LatticeEq(_, _) => Some((
            "LatticeEq `=` (lattice-family boolean track)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::Probe(_, _) => Some((
            "Probe `<=>` (direction probe)",
            (expr.span.start, expr.span.end),
        )),
        ExprKind::TypeAnnotation(_, _) => {
            Some(("TypeAnnotation `@`", (expr.span.start, expr.span.end)))
        }
        ExprKind::Interpolated(_) => Some((
            "Interpolated string (string building — non-monotone)",
            (expr.span.start, expr.span.end),
        )),
...
fn first_non_whitelist_field(field: &Field) -> Option<(&'static str, (usize, usize))> {
    if let FieldKey::Pattern(e) = &field.key {
        if let Some(r) = first_non_whitelist(e) {
            return Some(r);
        }
    }
