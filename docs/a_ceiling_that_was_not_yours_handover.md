...
## 9. 驗收回合（驗收方填，2026-09-09）

**結論：接受，零修補回合。** 五條射程全部成立，四條紅線全部保持。

### 9.1 三支衝突斷言 —— 裁定與修改（驗收方自己動手）

交付**逐條列出衝突、一字未改**，這正是 §4 紅線要求的。裁定如下。

**那三條斷言在本弧之前就是錯的，本弧只是把它們照亮。**
它們寫的是**當天那個答案的形狀**，不是探針存在的理由——而**該檔自己的檔頭
逐字寫著「these probes pin the PROPERTY, not either constant」**。
那個 property 是「**深輸入永遠不會殺掉行程**」；`Some(0)` 從來不是它。

〔量，交付二進位，三種形狀〕2000 層繭／8000 個 `!`／8000 項 `+` 鏈：
**三者皆 rc=1、stderr `Error: Parse Error: #stack_overflow`、無 signal、
零次 `overflowed its stack`** ⟹ **property 成立。**

*   `assert_deep_input_is_survivable`（r5／r6 共用）：`assert_eq!(code, Some(0))`
    改為 **`code.is_some()`**（死法是 signal，不是非零離開碼）＋
    「**做完了，或者說出它撞到什麼**」的二選一。
    **並註明這支 helper 刻意不釘離開碼**——它在 D63 前後都會是綠的，
    碰載體的是 R2 與新弧的 R1。**不是每一支探針都要斷言每一件事。**
*   `r2_…`：`assert_eq!(code, Some(0))` 改為 `code.is_some()` **＋
    `assert_ne!(code, Some(0))`**——比原本更強，它現在**正面釘住 D63 的方向**。
*   **並改名** `r2_a_deep_nest_is_a_clean_bottom_not_a_crash`
    → `r2_a_deep_nest_is_a_named_refusal_not_a_crash`。
    **一支叫「是一顆乾淨的 ⊥」而斷言「它刻意不是 ⊥」的測試，
    就是這整條線在治的那種不誠實回報。**
*   檔頭本弧敘事**一字不動**（那是當時的紀錄），另加一段標日期的裁定註記。
*   **非空洞性已驗**：原始缺陷（rc=134 ＋ `overflowed its stack`）仍會被前兩條擋下；
    `v0.45.0` 的形（rc=0 ＋ `_\|_`）會被 `r2` 的 `assert_ne!` 擋下。
*   結果 **15／15**。

### 9.2 交付對工單的三處更正

*   **(b) 成立，我寫得不精確。**〔查 `b2ba597`〕`main.rs` 命中 **6 行**
    ＝ 1 個 import ＋ **5 個 match 臂**。我寫「6 處」是行數不是呼叫點。
*   **(a) 成立**（本弧探針確實沒有 `#[ignore]`；那是 §N 模板的通用措辭）。
*   **(c) 兩邊各對一半。** 我的 §7.1 逐字是「**補丁前**的對照組」，不是補丁後
    ⟹ 交付所指的「不能同一次建置並存」打的不是我寫的那句。
    **但我那個括號把自己偵察時做的事描述錯了**：偵察用的是**條件式**補丁
    （只對含 `zzpanicz` 的輸入炸），所以對照組是在**補丁後的那顆二進位**上跑的，
    **那比補丁還原後再跑強**——它同時證明補丁沒有把別的東西弄壞。
    **本次不要求重跑**：③ 的證據不靠那個對照組站著，它靠
    `parser_thread_panic_is_not_a_depth_error` 這支常設單元測試站著。
    記在這裡是為了下一次工單把話寫準。

### 9.3 覆驗數字（驗收方自量，非轉抄）

*   **known-answer ＋ 對照**（交付二進位，離開碼直接取）：
    `eval '~%Math./add (1,2)'` → `3` rc=0；`eval '1 & 2'` → `_\|_ (%cause: #conflict)` rc=0。
*   **三種條件三個名字**（stdout／stderr／rc 三者分開）：
    ① 400 層 → rc=1、stdout 空、stderr `Error: Parse error: #stack_overflow`；
    ② `ulimit -v 300000` → rc=1、stderr `#host_resource_denied`；
    S4 `ulimit -v 60000` → rc=1、stderr `Error: #host_resource_denied`、**無 panic**。
*   **G4 手驗**：求值期仍為 `big: _\|_ (%cause: #stack_overflow)`、`status` rc=0。
*   **`oo lint` 沒有回歸**〔對照 `v0.45.0` 二進位〕有 WARN 的檔兩者皆 rc=1、
    乾淨的檔兩者皆 rc=0；壓力下由 `diagnostics: 0`／rc=0 變為 `1 error`／rc=1。
*   **身分紅線**（新舊二進位各造一個 `x: 0` 倉，物件 CAID 逐字比對）：
    根 **`31745ef0…`** 兩邊皆在、標準根 **`7038e250…`** 兩邊皆在、**物件 3**、
    `layout=5`、`.oo/objects.format` **`encoding=5`**。
    **第三個物件兩邊不同 ＝ 帶時間戳的 commit**，〔對照〕同一顆二進位造兩次
    第三個物件同樣不同 ⟹ 與本弧無關。
*   **跨版本**：真 `v0.44.0`（自報 `oo v0.44.0`）讀本版寫的倉，
    `status` rc=0、`log` rc=0、**再提交一次 rc=0**。
*   **交叉編譯** `x86_64-pc-windows-gnu`：**0 error**。
*   **符合性**：**162 vectors, 162 pass, 0 fail**；0 新向量，逐字記帳。
*   **本弧探針 10／10**（含六支紅全部轉綠，無 VOID 路徑）；
    `limit_you_cannot_choose` 10／10；`a_limit_you_cannot_catch` 15／15。

### 9.4 交付報的那個 flake：**是舊的，而且我沒有把兩個讀數說成同一個**

`r4_two_concurrent_discharges_both_survive`（Q-040 弧）。
〔量，分母皆為 10〕**本樹 8／10**；**`v0.45.0` 標籤樹 9／10**。
⟹ **本弧之前就在抖**。兩個各 10 次的樣本**分不出 0.9 與 0.8**，
故只記「兩邊都不穩」，**不宣稱「沒有變化」**。已入 Inbox，非本弧。
