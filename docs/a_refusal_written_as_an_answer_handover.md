...
**R-1 本身修對了；但驗收 R-1 時量到第二件，而它的錯在驗收方 ⟹ 開修補回合 R-2。**
diff 純度：三檔（`main.rs`／`storage.rs`／本檔報告），探針兩檔一字未動。
〔量，R-1 建置 `7187b95`，以行為判準確認二進位：對最新倉回 `already layout=5 and encoding=5`〕
r9 綠；本檔其餘全綠；`layout=5`／`encoding=4` 由再跑一次 `migrate` 完成為 `encoding=5`。

### 11.1 R-2：代價名單不完整，而那是驗收方給的數字

工單 §1 M5 與 Q3 寫著「〔量，真二進位〕layout 2 → 5 之後**實際鎖在門外的是 v0.40.0–v0.43.0**」。
**那個量測只跑了 v0.40.0 以上的二進位**——下界是量測範圍的邊，不是事實的邊，而驗收方沒有印出分母。
交付照抄了它。〔量，本次，手上每一個真二進位（v0.20.0、v0.22.0–v0.27.0、v0.32.0–v0.44.0），
倉**由舊引擎建立**，遷移用交付建置，逐引擎在遷移前後跑 `status`〕：

| 建立者 | 宣告 | 遷移前打得開 | **遷移後被鎖在門外** | 交付的句子說 |
| :-- | :-- | :-- | :-- | :-- |
| v0.22.0 | `layout=2`／`encoding=3` | v0.22.0 … v0.44.0 | **v0.22.0 … v0.43.0** | v0.40.0 through v0.43.0 |
| v0.27.0、v0.33.0 | `layout=2`／`encoding=4` | v0.26.0 … v0.44.0 | **v0.26.0 … v0.43.0** | 同上 |
| v0.36.0、v0.39.0、v0.40.0 | `layout=2`／`encoding=5` | v0.36.0 … v0.44.0 | **v0.36.0 … v0.43.0** | 同上 |

（v0.28.0–v0.31.0 手上沒有，落在每一段之內。）⟹ **被鎖在門外的集合是兩軸的函數**：
打得開 layout 2 的是 v0.22.0 起，而其中還要讀得懂那一份編碼（3：v0.22.0 起；4：v0.26.0 起；5：v0.36.0 起）。
交付的表只從 v0.40.0 起看 `STORE_LAYOUT_VERSION`／`STORE_LAYOUT_MIGRATABLE_FROM`，漏了 v0.22.0–v0.39.0 也寫、也開 layout 2。

**⚠ 驗收方的探針也放過了它**：r6／r7 的 `names_the_boundary` 接受「v0.43.0 或 v0.44.0」，
而交付的句子結尾「oo v0.44.0 and later still open」**就含 v0.44.0** ⟹ 判準接受了錯的那一端。
新探針 **r11**：三個起點各要求點名**最舊**的被鎖者（v0.22.0／v0.26.0／v0.36.0）與 v0.43.0；
唯讀 `.oo` 使只量句子、不寫入。R-1 建置上紅。

**修補的不變式**：*代價句點名的，是「遷移前打得開、遷移後打不開」的**全部**參考引擎，
依這個儲存的兩份宣告算，不是只依 layout。* 名單的來源你選（逐標籤讀常數、或一張由真二進位量出的表），
**但報告要附你的表與它的來源**，並回答：**v0.20.0／v0.21.0 的裸數字宣告**起點，名單是什麼？

### 11.2 已獨立複驗的（R-1 建置）

*   本檔 17 支（＋r11）：**16 綠 1 紅**（r11），無 `VOID READING`。
*   conformance、全樹 ×3 留到 R-2 驗收一次跑（R-1 回報 240 target／2278／0、cargo exit 0，未複驗）。
...
**受理。** 兩個修補回合，**兩個成因都在驗收方**（R-1 射程只寫了半個不變式；R-2 量測範圍被寫成事實的邊）。
diff 純度：R-2 只動 `main.rs` 的代價句與本檔報告；兩個探針檔自驗收方最後一次提交後**一字未動**；未動 `bn_serial`；未 `rustfmt`。

### 13.1 已獨立複驗的（R-2 建置 `9b6ec16`，以行為判準確認二進位）

*   **代價句對照真二進位**：七個起點，各由**真的建立者**造倉，以 v0.20.0、v0.21.0（`-plus` 建置）、v0.22.0–v0.27.0、
    v0.32.0–v0.44.0 共 21 個真二進位於遷移前後各跑 `status`。**七個起點的句子與實測區間逐一相符**：
    裸 `2` v0.20.0…v0.43.0／裸 `3` v0.21.0…／`layout=2` 且 `encoding=3` v0.22.0…／`=4` v0.26.0…／`=5` v0.36.0…／
    `layout=3` v0.42.0…v0.43.0／`layout=4` v0.43.0；**遷移後七者都只剩 v0.44.0 打得開**。
    〔量測範圍〕v0.28.0–v0.31.0 與 v0.2.55–v0.19.0 未在手；裸 `1` 起點未量。
*   探針：本檔 **17／17**（r9、r10、r11 皆綠），`nothing_here…` **7／7**，無 `VOID READING`。
*   全樹 ×3（`--release --no-fail-fast --jobs 1 -- --test-threads=1`）三輪一致：
    **240 target**（237 Running＋3 Doc-tests）、**2279 passed／0 failed**、`^error` 0、cargo exit 0、無失敗測試名。
*   conformance **162／162**；known-answer `3`／`4`；身分紅線 `x: 0` → `31745ef0…`、`v: 1 + 1` 與 `v: 1+1` → `f4f32e7b…`、
    標準根 `7038e250…`——**三者未動**。

### 13.2 殘留（不開修補回合，已入帳）

*   `names_the_boundary`（r6／r7）的判準仍寬（接受任何含 v0.44.0 的句子）；**r11 已取代它的完整性部分**，r6 仍守「動手之前」。
*   `discover` 丟掉落地失敗（Q1(c)）已入 Inbox；`log`／`refine` 的第二次讀不修，理由見 8.4。
*   `engine_or_ephemeral` 依賴 `init` 的寫入順序，由 r10 釘住觀測。
