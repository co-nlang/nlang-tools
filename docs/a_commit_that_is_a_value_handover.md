...
**受理。** 兩個修補回合；三項中兩項的源頭在驗收方（同一個錯的量法），一項在交付（v2 提交位址只比摘要）。
diff 純度：R-2 只刪了 `identify` 的特例與它唯一的輔助函式——`disc.rs` 與開卡前**逐位元組相同**；兩個探針檔自驗收方最後一次提交後一字未動。

### 13.1 已獨立複驗的（R-2 建置 `8954d72`，以行為判準確認二進位：`identify` 提交本體回 `…9ceb1d51…`）

*   探針：本弧 **13／13**（含 g5、r7），Q-055 檔 **17／17**，無 `VOID READING`。
*   全樹 ×3 三輪一致：**241 target**（238 Running＋3 Doc-tests）、**2292 passed／0 failed**、`^error` 0、cargo exit 0、無失敗測試名。
*   conformance **162／162**；known-answer `3`／`4`；值的位址 `x: 0` → `31745ef0…`、`v: 1 + 1` → `f4f32e7b…`、標準根 `7038e250…` **皆未動**。
*   〔真二進位〕v0.44.0／v0.58.0 對 `layout=6` 以「宣告看不懂」拒絕；**v0.58.0 讀寫一個由新引擎續寫的 `layout=5` 儲存**
    （新提交仍是 v1）`status`／`log` 皆 rc=0。
*   遷移夾具副本：代價句先於動作、點名 v0.44.0 through v0.58.0；`HEAD` 不動；舊 refine 授權印 `unattested`；遷移後的新提交是 v2。
*   40 筆 `oo log`（R-1 建置量）：layout 5 約 75 ms、layout 6 約 140 ms（1.9×）。

### 13.2 殘留（不開修補回合，已入帳）

*   舊式提交位址依 Unicode 表——已寫進 `REAL_03` §6.10 並自陳。
*   `store_codec.rs` 測試模組前多了一行空白（R-1 留下），無害。
