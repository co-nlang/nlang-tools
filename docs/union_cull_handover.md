...
**PASS——零代修(第十九例)**。交付 commit `9b2129b`。

- **Diff 純度** ✓:探針檔僅 8 個 `#[ignore]` 移除;value.rs 新
  helper `primary_bottom_from_culled`(`min_by_key` 首見最小=相遇序
  最左 ✓,normalize_union 本體未動);T1 淺 force 迴圈 cap 32
  (詞法弧 ≤32 先例同款,超限=倖存不剔=保守正確);T2 分流;
  T3 臂 sort/cap/nondistrib 一行未動(雷區遵守)。被剔 ⊥ 之 effect
  併入累積(保守向,不漏報)。
- **獨立重跑** ✓:探針 15/15、workspace **1137/0/3**、conformance
  **110/110**(L2-69~71 翻綠)、語料非 pending 74/0(67+7)。
- **對抗全正**:blur 支+⊥ 支+值支混(`#blur | 1`,相遇序保全、
  §3.2.2 #5 全款)/混階全 ⊥ 原樣訊息/rank2 違規勝 rank5/雙側剔後
  `=` `#true`/剔+去重/root 全 ⊥ 異階 `#divergent` 原樣(T2 出口
  接手 Join 直鑄雙 ⊥)。
- **靜止環分歧形受擾申報(驗收方量測,工單預留條款)**:兩語境
  現**皆** `9`——導航投影 force 後,惰性投影把靜止環判成
  ⊥ #divergent(SPEC_12 家族既有誤判)而**依法剔除**,`_` 支自
  結果靜默消失(法理想=`9 | _`)。剔除律執行正確、病灶在上游
  分類——歸 SPEC_12 家族裁定候選,帳已更新(升級註記:誤判如今
  有觀測後果,非僅顯示異)。
