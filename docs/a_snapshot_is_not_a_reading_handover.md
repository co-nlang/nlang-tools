...
## 13. §12 的「通過」被規格收尾期間的量測推翻(2026-08-10)

寫 REAL_04 §4 的 `#blur` 投影款時,我遵循「不寫沒量過的 MUST」去驗引擎,
結果**推翻了 §12 的驗收結論**。

### 13.1 R-6 的核心要求沒有達成

多記錄結構**已實作**(`BlurDetail.co_horizons` / `merge_set` / `all_records`),
但 CHS 把**主記錄**(`cause` + `horizon`,取自左運算元)與 `co_horizons` **分開餵**,
於是「誰當主記錄」進到了雜湊:

```
控制  A 單獨  d0967392…      B 單獨  e3d8e76a…      (兩側非空且相異)
      A & B   7967db7f…      B & A   c64f900f…      ⟹ 不交換
```

**代修 M4**:CHS 必須雜湊**全集合的正準序**;主/副之分只保留給 `%cause` 投影與顯示。

### 13.2 為什麼我的探針沒抓到——這是本弧最該記住的一件事

原 R6 把運算元寫成**座標**(`x: A / y: B / p: x & y`)。座標抵達 meet 時是**未 force 的 thunk**,
於是那一對**從來沒有走到 `(Blur, Blur)` 臂**,而是命中 O47 的吸收臂
`(Blur, _other) => blur`——原樣回傳其中一個運算元。兩個方向因此得到相同的摘要,
斷言通過,**而關於記錄聯集的事情一件也沒有被測到**。

**缺的是控制組。** 一個關於「某個運算」的相等斷言,需要一個「該運算確實發生了」的見證。
補上 `ab ≠ a_alone` 與 `ab ≠ b_alone` 兩個控制之後,性質立刻紅。

**新常設規則**:
> **斷言「f(x,y) 具有某性質」的探針,必須先斷言 f 真的被套用了**
> ——通常是「結果與任一運算元皆不相等」。否則一個把 f 實作成投影
> (回傳其中一個引數)的引擎會讓所有對稱性斷言免費通過。

這與既有的「掃描全集型探針首位須放 control」是同一條規則的另一個面:
**前者防走訪器靜默失效,本條防運算根本沒被執行。**

### 13.3 連帶

* **P4 的字面釘不受影響**(單一 blur,不經合併)。
* §12 其餘各項(M1/M2/M3、跨版本、對抗、conformance/genesis)**維持成立**,
  各自有獨立量測。
* **v0.17.0 不切**,待 M4。
