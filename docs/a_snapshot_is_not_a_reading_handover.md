...
### 11.6.1 裁定(用戶 2026-08-10):走 (i)

**evolve 時把 partial 本體寫進 CAS。** evolve 從「寫 0 個物件」變成會寫;
未提交者為不可達物件,由 v0.2.53 的本地 GC 回收(SPEC_08 §6.2:使已記錄事實
不可達是特權操作——此處回收的是**從未被提交**的物件,與該條治的不是同一件事,
與 v0.2.54 廣告自動清除同一先例)。**O45 因此保持可能。**

### 11.10 釘的字面值一律延到最終驗收才釘

M1 會**再一次**移動每個 blur CAID(身分輸入由「內嵌 partial」改為「partial 的 CAID」)。
因此本次代修**不釘任何字面 digest**——會被釘住的量在一次代修內移動兩次,
現在釘等於埋一顆保證誤爆的地雷。

作法:代修期間全部改為**關係型斷言**(跨行程相同、兩者相異、族間序);
**最終驗收通過時,由驗收方一次把字面值釘回去。**

---

## 12. 最終驗收(驗收方,2026-08-10)——**通過**

代修交付 `eb722f4`。**交付範圍只含引擎九檔,未碰任何探針。**

### 12.1 三件必修

| | 驗法 | 結果 |
| :-- | :-- | :-- |
| **M2** 還原守衛 | `Cargo.toml` diff 逐行還原;全樹 grep `disable_recursion_limit`/`unbounded_depth` | **零命中** |
| **M3** 當機 fixture | `depth: 4294967295` × 5000/8000/12000,與交付前二進位 A/B | **兩側全為 0** |
| **M1** partial 以 CAID 入身分 | `.oo/staged` 的 JSON 深度與大小 | 見下 |

`.oo/staged`(`~%Config.max_unification_depth: 30`,長度不同的加法鏈):

| 項數 | 交付前 | 第一次交付 | **代修後** |
| :-- | :-- | :-- | :-- |
| 200 | 深度 10 / 649 B | 517 / 19,874 B | **10 / 826 B** |
| 1000 | 10 / 654 B | 2,917 / 113,503 B | **10 / 832 B** |
| 5000 | 10 / 657 B | 14,917 / 591,432 B | **10 / 834 B** |

**深度與運算式大小無關地固定在 10** ⟹ serde 的 128 層上限永遠咬不到,守衛不必拆。
大小是 +~177 B 的**常數**(CAID 字串),不是線性。
`objects` 於 evolve 後為 **1** ⟹ 裁定 (i)(本體於 evolve 落 CAS)已落地。

### 12.2 閘

| | |
| :-- | :-- |
| workspace(獨立重跑) | **1837 / 0 / 3** |
| 本弧探針 | 15/15,**五次重跑穩定** |
| conformance | **143 / 143** |
| genesis | **11 / 11** |

計數:1836(代修前含驗收方復原的兩支釘)+ 1(§12.5 新增的 P4)= **1837**。

### 12.3 對抗:裁定 (i) 引進的新失效模式

blur 指向的 CAS 物件**可能不在**。實測:evolve 後刪光 `.oo/objects`,

* `status` 照常列出該 blur,**`%caid` 逐位元組不變**,退出碼 0;
* `run --observe` 同上。

⟹ **身分不依賴本體取得回來**,這正是內容定址該有的性質。
〔掛帳給 O45:當 `%partial` 變成可觀測時,「本體不在」需要一個說法;今天沒有,
因為今天沒有人看得到它。〕

另:`oo run`(不 evolve)仍寫 **0** 個物件 ⟹ **觀測保持不落地**。

### 12.4 跨版本,雙向,控制已武裝

| 方向 | 結果 |
| :-- | :-- |
| 新引擎 ← v0.16.0 舊倉 | 讀得到,root `3a4a2c85…` |
| **舊引擎 ← 新倉** | **`#object_undecodable: … missing field 'salt'`** |
| 控制:舊引擎 ← 舊倉 | 正常解碼 |

舊引擎的拒絕是**具名的、說得出為什麼的、不當機的**,而且措辭是
「present but cannot be decoded (integrity unknown)」——**沒有謊稱完整性受損**,
正是「線上說得出為什麼」(v0.2.55)那一弧建立的形狀。破壞性條目 #11 的雙向行為如上。

### 12.5 字面值釘回去,以及一個被實測推翻的推論

* **P4(新增,`snapshot_not_a_reading`)**:深度側 blur 的 CAID 字面值
  `d0967392…`。**這支釘在本弧之前造不出來**——那時它每次執行都是不同的 64 個字元。
  釘住它是「O42 交付了什麼」最短的說法。
* **P2(`name_points_at_remedy`)**:字面值釘回 `4120193908e8…`,並保留關係型斷言。

**§11.10 的推論在這一格是錯的。** 我寫「M1 會再一次移動每個 blur CAID」,
據此把 P2 去字面化。實測:燃料側那個 blur **沒有 partial**,`CAID(None)` 與
內嵌 `None` 雜湊相同,值**從未移動**——與第一次交付所算的完全一致。
推論對深度側成立、對燃料側不成立。**去字面化本身仍是對的**(手上沒有代修時無從得知),
但理由已寫進探針註解,免得下一個人繼承一條只對一半的經驗法則。

### 12.6 獨立覆核(不經探針 harness)

* 同源三個全新倉 → root **全等** `d21875a8…`。
* `p == q` 兩個逐字相同的 blur → **`#true`** ⟹ SPEC_08 §3.2.2 第 6 款 (a)
  自訂立以來第一次可達。

### 12.7 未結:規格收尾(驗收方)

REAL_03 §7.3(六項、timeout 排除理由改寫、`#eager` 筆誤)、SPEC_01 §2.4.1 第 5 項
排序鍵(含**修訂那條「禁止以 CAID/digest 作排序鍵」的 MUST NOT**)、SPEC_03 §90 按語、
SPEC_08 §3.2.2 第 6 款、REAL_04 §4 blur tie-back、ENGINE_SYNC、CHANGELOG。
