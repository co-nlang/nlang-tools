...
### 9.6 R-1：用戶裁 乙——宿主層的環自己一個標籤

**用戶 2026-09-15 裁定：乙。** §9.4 的兩條路取「另立一個宿主層的兄弟標籤」。

#### R-1 的不變式

> **檔案系統的符號連結環，與 n/ 定義之間的純引用環，不得以同一個成因作答。**

**不指定措辭**，但驗收方**指定一個預設值**，理由與依據如下；認為該用別的字就在 §R.3 說。

* **`#path_cycle`**，依登記簿既有的 `<domain>_cycle` 體例
  （`#static_cycle`／`#refinement_cycle`；〔查〕`path_cycle` 在全 spec 零命中）。
* **軸·載體 ＝ 原因·Top**，與本弧另一個纖維 `#unreadable` 同格。
  〔查〕今天 Top 載體只有 `#static_cycle`／`#no_coordinate` 兩個成員，**兩個都是「來歷」且皆標非錯誤**
  ⟹ **本弧造出登記簿的頭兩個「原因·Top」。**
* **改名便宜**：〔量，驗收回合〕成因不入址（兩個只有纖維不同的宇宙共用根 `c5364f7e…`）。

#### 不要動的

* **n/ 自己的 `#static_cycle` 一個字都不要動**（R4 的對照組 1 釘住它）。
  本弧要的是**停止借用**，不是搬家。
* `#unreadable`、ENOENT／ENOTDIR／斷連結、store 邊界、`write_file`／`append_file`（O87）。
* 列印規則今天以「是哪一個 cause」列表判斷要不要印註解層——**那是個案不是類別，而類別是「軸」**
  （原因要印、來歷不印）。**但 `TopCaused` 今天不攜帶軸** ⟹ **本弧不改**，已記在 §9.3。

#### 探針 R4（驗收方所加，現況紅）

`r4_a_symlink_loop_and_an_n_cycle_do_not_share_a_cause`，**三個對照組都在同一次執行內**：
n/ 純引用環仍須答 `#static_cycle`（證明沒有搬錯家）／EACCES 仍須答 `#unreadable`／
REACH 守衛證明符號連結環真的產生了一個成因。
**本弧探針基線更新為 6 綠 1 紅。**
...
### 9.7 R-1 複驗與收弧（驗收方，2026-09-15）

**R-1 通過。Q-046 驗收完成，一個修補回合。**

修法是把 `ELOOP` 的成因換成 `path_cycle`，**並順手刪掉 `no_answer_top` 裡的 `static_cycle` 特例分支**
——那個分支存在的唯一理由就是借用，借用停止之後它是死重。**刪比改好，這一格刪對了。**

〔量，本修補二進位，**目標一格、對照八格，全部在同一次量測內**〕

| | 結果 |
| :-- | :-- |
| **ELOOP 的成因** | **`#path_cycle`** ✅ |
| EACCES 的成因 | `#unreadable` ✅ 未動 |
| ENOENT／ENOTDIR／斷連結 | `#false` ✅ 三格皆未動 |
| 可讀 | `#true` ✅ |
| store 邊界 | `_\|_ ;; %cause: #store_boundary` ✅ |
| **n/ 純引用環** | **`#static_cycle`** ✅ **沒有搬家** |
| 開放缺欄 | `#no_coordinate` ✅ |
| n/ 環的列印形 | 裸 `_` ✅ 未動 |

* 探針：**7／7**
* 全工作區 ×3：**231 targets／2212 passed／0 failed**，三輪皆 `cargo_rc=0`
* conformance **162／162**；跨版本雙向 rc=0
* 身分：`31745ef0…`／`7038e250…`／物件 3／`layout=5`／`encoding=5`
* 探針檔跨交付與修補 **0 行改動**

### 9.8 規格結案（驗收方，已做）

`nlang-spec/spec/zh_TW/TAG_REGISTRY.md`：

1. **§1.2 新增兩個條目** `#unreadable` 與 `#path_cycle`，皆 **原因·Top**，
   置於 `#host_resource_denied`／`#internal_error` 之後（同為宿主面）。
2. **§2.3 載體：Top 由 2 個改為 4 個**，並加一則按語：**本格的軸分成兩半**
   ——前兩者是**來歷**（皆非錯誤、消費即蒸發），後兩者是**頭兩個「原因·Top」**；
   **載體相同時，軸決定操作者要不要動手。**
3. **§2.7.6 新設命名裁定**（Core Requirement），四條 MUST NOT：
   不得以值回報「沒有答案」（那是排中律）／不得以 ⊥ 回報「世界不肯說」
   （`⊥ → #true` 不是合法收窄；ⳇ 且 ⊥ 是 meet 零元）／不得以 `#blur` 回報
   （快照義務 ＋ 無效補救）／**宿主層的環不得借用 n/ 層的環之名**。
   依 §2.7 體例附**論證性量測**（改動前四種宿主狀況一個答案、改動後逐格、
   成因不入址而隨物件旅行、`.%cause` 兩端皆通），並逐字寫明
   「拿掉這些量測，上列四條就只剩命名偏好」。
