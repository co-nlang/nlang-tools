...
獨立重測:探針 diff 僅 8 個 `#[ignore]` 移除、斷言原封;探針 15/15;
workspace **800/0/3**(785+15,吻合);語料 **72/0**;conformance **51/51**。

對抗性邊界戳刺(工單外,全數健康):
- 巢狀 tuple 參數 `((x, (y, z)) -> …)`:維持舊行為 `_`(守門正確,未偷跑)。
- **Union 引數逐支解構**:`((1,2) | (3,4)) |> tf` → `3 | 7`(分配先於分派,
  解構自然逐支;單參對照組 `1 | 3` 同構)。
- Unit 引數 → ⊥ #conflict(非 tuple 形正確拒絕)。
- 重複參名 `((x, x) -> x)` → 後者勝(arg.1),與柯里側內層遮蔽嚴格對偶。
- 參名 `it` 顯式遮蔽整包綁定(`((it, y) -> it + y) (3,5)` = 8)——顯式名勝,
  與柯里側 `(it -> …)` 行為一致。

觀察(非違規,記錄在案):
1. tuple 形判定只驗 data 軸(恰 `0..k-1`、精確 arity);帶其他軸
   (rules/types)但 data 軸吻合的 combo 亦可解構——寬鬆但與工單條文
   字面一致,實務中此形罕見,不改。
2. 參名恰為 `it` 時遮蔽整包綁定(見上)——語義自洽,規格未著墨,留跡。
