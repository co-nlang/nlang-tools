...
## A4. 驗收回合 4（驗收方填，2026-08-31）

**A3.2 修好了。而 R3.1 裡那個時間戳回退，是我的探針逼出來的——請把它拿掉。**

### A4.1 A3.2 已關（真二進位複驗）

〔量，v0.40.0 標籤建置造倉（known-answer `3`），本弧建置讀〕
三顆舊 commit → 本弧引擎 commit 一次 → `oo gc --grant gc`：

```
log:            new4 old3 old2 old1
gc:             removed 0 objects, freed 0 bytes
log after gc:   new4 old3 old2 old1
inspect oldest: CAID: hash:sha256:v1:cefec399…      （仍在）
```

**對照回合 3 的同一組數字**（物件 7→3、log 3→1、兩顆 `CAID not found`）
⟹ **資料遺失已關。** `ancestor:` 記 digest 是對的解法。

### A4.2 但請拿掉 `predecessor_by_timestamp`

**它的存在理由是一個不可能發生的狀態，而那個狀態是我的探針造出來的。**

〔讀〕`previous_commit` 在「這顆 commit 沒有 `parent`、沒有 `ancestor:`、
**也沒有任何 ○ 點名它**」時，落到 `predecessor_by_timestamp`：
**掃遍 `objects/sha256/` 每一個檔、逐個解碼、挑時間戳嚴格小於自己的最大那顆。**

**那個狀態不可能出現**（你 R3.1 自己也是這樣描述它的）：
本弧引擎寫的 commit **一定**有一顆帶 `ancestor:` 的 ○；
本弧之前寫的 commit **一定**帶 `parent`。**兩條路都不落到那裡。**

**它只在我的舊 G7 剝掉註記之後才成立，而那支探針模型是錯的**（見 A4.3）。

**要拿掉的理由不是「用不到」，是「它做的事正是本弧在反對的事」**：

*   **§1.7.1 逐字：本地圖是「你造的」。** 祖先是一條**被宣告**的邊。
    從掛鐘順序**推導**出來的祖先，不是同一個東西。
*   `SPEC_10` §3.1 有一條 **前驅不得由到達順序決定（MUST NOT）**，逐字理由是
    「**以到達順序裁決等於讓歷史取決於排程**」。那一款規範的是 Savepoint 的前驅、
    不是 commit 的祖先，**所以我不說它違反了那一條**——但**它違反的是同一句話背後的那件事**。
*   而時間戳本身撐不住：偵察 Q2 已量到它是**毫秒**、相鄰兩次提交實測差 **54 ms**、**同秒可撞**。
*   〔讀〕它是**每次走訪一步就掃一次全 store 並解碼每個物件**，而 log／squash／`gc::mark`
    現在都走這條路。

**⟹ 射程只有一件：刪掉 `predecessor_by_timestamp` 及其呼叫點，回到「找不到就 `None`」。**
探針側 `local_gc`／`verdict_must_gate` 的 walker 你已經沒加時間戳回退，不用動。

### A4.3 這一件整個是我的鍋，而且工單早就寫過正確答案

舊 G7 用**本弧引擎**造 commit，再把 `commit:`／`ancestor:` 從框上剝掉。
剝完剩下的是 **`parent: None` ＋ 無 ○ 點名**——**一個真實世界產生不出來的狀態。**
**探針模型錯了，而交付方為了讓它綠，加了一個猜的機制。合理反應，錯的前提。**

**更難看的是工單早就寫對過。** §4 的「逐字記下沒有探針的兩件」第 2 條原文：

> 跨版本混鏈**造不出來**——`cargo test` 裡的二進位就是新的，做不出舊形狀的 commit
> ⟹ **由驗收方在驗收時用 v0.40.0 標籤二進位造倉、新引擎讀。**

**我寫了這段，然後還是去寫了一支試圖在 `cargo test` 裡做這件事的探針。**

### A4.4 G7 已換掉

`g7_gc_keeps_every_commit_reachable`：六顆 commit、`gc`、斷言**六顆都還 inspect 得到
且 log 仍是六列**。**綠**，且它釘的是**讓那個回退變成不必要的那條性質**——
**走訪與回收必須走同一條邊**。回合 3 的資料遺失若發生在本引擎自己寫的倉上，這一支會紅。
**跨版本那一半仍是手量**，如工單原本所寫；A4.1 就是本回合的那次手量。
...
## A5. 驗收通過（驗收方，2026-08-31）

**四個修補回合之後，全部通過。**

| 項 | 結果 |
| :-- | :-- |
| 全跑 ×3 `--no-fail-fast --test-threads=1` | 每次 **exit 0／221 target／2127 passed／0 failed／`^error` 0**，**失敗測試名為空** |
| conformance | **162／162** |
| 身分紅線 | `x: 0` 根 `31745ef0…`、**3 物件**、標準根 `7038e250…` |
| $H_1$（N ＝ 3／10／20） | **0／0／0**（$V=2N$、$E=2N-1$、$C=1$） |
| 跨版本混鏈（**真 v0.40.0 標籤二進位**造倉） | `new4 old3 old2 old1`；`gc` **removed 0**；最舊那顆仍 inspect |
| rollback（基線 vs 交付） | 兩邊皆 `final gen1`、各恰一行 `abandoned`——**逐字相同** |
| squash 對被回溯掉的 base | 兩邊皆 `not an ancestor`——**逐字相同** |
| 弧探針 | **9／9** |
| 探針純度 | 交付方只拿掉兩行 `#[ignore]`；G5／G6／G7 四個回合中**一字未動**；未 rustfmt |

**回合 4 的射程（刪 `predecessor_by_timestamp`）已確認完成**：全樹該名字 **0 命中**。

### A5.1 交付方做對而值得記的三件

1.  **回合 1 抓到工單漏掉的第六個讀者**（`gc::mark`），並說明了不補的後果是**無聲的資料遺失**。
    那是**替工單補洞**，不是順手改動。
2.  **R2-3 照要求先量再決定**：`abandoned` 過濾量完確認多餘、移除，**沒有留成死碼**。
    工單那句「量完再決定移不移除」被逐字執行了。
3.  **每一次偏離都自陳**：回合 1 的父指向、回合 3 的時間戳回退，
    **都寫在 N／R 節裡而不是藏著**。回合 2 與回合 4 的兩則裁定，
    都是因為交付方把偏離寫出來才拿得到的。**公開的偏離讓錯誤變便宜。**

### A5.2 規格收尾（驗收方，已完成）

`SPEC_10` §3.1 新增三款（提交亦產生 Savepoint／前驅與祖先是兩個關係／祖先不得由時間導出）；
`SPEC_08` §6.2 §578 射程更正；`REAL_01` §4.2 佈局補註記。
**四條新條文引擎全部已兌現**（R1／G5／G6／G7 逐一釘住）⟹ **不開 Inbox 列**。
**無合規向量，逐字記帳**——性質全在儲存容器與 CLI 走訪，語料形式無法表達；**162 對 162 不變**。
