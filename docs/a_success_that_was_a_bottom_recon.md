...
## B. 與 D46 對帳（驗收方，2026-08-31，用戶提問）

用戶問：**「為何是增 §2.3.1 但 §2.3 好像沒修一樣，前一行的『語義』看起來也是舊的描述？」**
並判斷：**「現在要裁的，就是 §4.1.2 在講的東西。」** ——**對帳結果：兩點都成立，而第二點更強。**

### B.1 為什麼 §2.3 沒被動：D46 的後果清單裡沒有它

〔讀 `STATUS` D46〕該裁定逐字列了**三個直接後果**：
① 引擎違反它（已量到的無聲資料遺失）／**② `SPEC_10` §1.1 #1 須改寫**／③ D45 的射程註記須補一句。

〔複驗〕**② 付了**——§1.1 #1 今天帶著「**⚠ 射程更正（2026-08-28 裁定 D46，Q-013）**」那段。
**而 §2.3 從頭到尾沒有出現在那張清單上。**

⟹ **不是漏改，是漏列。** 清單是照「D46 讓哪一句變成假的」造的，而**沒有人去看 §2.3**。

### B.2 §2.3 的兩行都舊，而且舊法不同

*   **「語義」那行**：「將當前 Staged 的內容與當前 Commit $C_n$ **合併**」
    ——**D46 之後 commit 不是「合併並強制求值」**，它是**投影且保留 `#pure` 的 thunk**
    （§2.3.1 逐字）。而 Q-014（D48）之後 Staged 也不再是一個被改寫的格子，是注入集合的 fold。
    **這一行寫於兩者之前。**
*   **「原子性保證」那行**：「若合併結果產生 `_|_`，則**提交失敗**」
    ——**這一行就是 §4.1.2 在 2026-08-06 拆掉的那條規則，活在另一節裡。**

### B.3 ⚠ 決定性的一件：§4.1.2 已經量過它為什麼永遠不開火

〔讀 §4.1.2 開頭〕「本條原文作『若自動收斂產生 `_|_`，**提交必須立即終止**』」
——**與 §2.3 那句是同一條規則的兩種措辭。**

〔讀 §4.1.2 的論證性量測（2026-08-06）〕逐字：

> 本條原文之「中止」在參考實作中**有實作卻抓不到任何東西**——它只在合併結果**不是 Combo**
> 時觸發，而**兩個 Combo 相 meet 恆為一個帶 `_|_` 欄位的 Combo**。

⟹ **「(a) 還是 (b)」這一題,§4.1.2 早在十個月前就答過了：實作讀的是 (a),而那正是它沒有牙齒的原因。**
**§2.3 是同一條規則、同一個機制、同一個死法——只是沒有人把它一起拆掉。**

⟹ **驗收方上一則說「待裁兩件」是多算了一件。** §2.3 不是與 §2.3.1 打架的活條文，
**它是一條已經被拆解過、但拆解沒有覆蓋到的殘留**。**要做的是編輯，不是裁定。**

### B.4 ⚠ 驗收方在 A.2 又說錯一次，就地更正

A.2 逐字寫「A 類**falls through 每一條條文**」。**那句話也是錯的**——
驗收方只查了 §2.2.1／§2.3，**沒有讀 §4.1.2 自己的款**。

〔讀 §4.1.2〕**回報的內容（MUST）** 逐字：

> 引擎**必須**指出 `_|_` 落在哪些座標，而**不僅回報「成功」**。
> **靜默地固化一個帶矛盾的狀態，等於讓操作者在不知情下做決定。**

**那一句逐字描述的就是引擎今天做的事。** 條文存在，而且是 MUST。

### B.5 ⟹ 要裁的只有一件，而且很窄：§4.1.2 的射程

〔讀〕**本條之射程（MUST NOT）**：

> 本條治的是 §4.1.1 之**自動收斂**路徑——亦即操作者**未做任何決定、僅因並發時序**而落入 `_|_` 的場合。
> **顯式地固化一個含 `_|_` 的內容不在本條射程內。**

**那是一個二分，而真實的案例落在它的縫裡：**

| | 操作者知道有 ⊥ 嗎 | 誰造成的 | §4.1.2 射程 |
| :-- | :-- | :-- | :-- |
| 並發時序落入 ⊥ | 不知道 | 排程 | **在**射程內 |
| **`c: c + 1` 然後 commit** | **不知道**（`status` 逐字印 `c: c + 1`） | **操作者的定義** | **縫裡** |
| 明知是 ⊥ 仍要固化 | 知道 | 操作者 | **不在**射程內 |

⟹ **中間那一列，操作者做了決定，但那不是一個知情的決定，因為在他決定之前沒有任何一面告訴他有 ⊥。**
而 §4.1.2 自己的理由句正是為這種情形寫的：「**靜默地固化一個帶矛盾的狀態，
等於讓操作者在不知情下做決定。**」

**要裁的一句話**：§4.1.2 的射程二分是否要改成**以「操作者是否被告知」為軸**，
而不是以「並發 vs 顯式」為軸。若是，中間那一列進射程，
**回報的內容（MUST）當場就適用，本卡從「要不要立法」變成「兌現一條既有的 MUST」。**

### B.6 對帳的產物（不需要裁定的部分）

*   **§2.3「原子性保證」那行是殘留**，與 §4.1.2 是同一條規則 ⟹ **編輯性移除或改指向 §4.1.2／§2.3.1**。
*   **§2.3「語義」那行寫於 D46 與 D48 之前** ⟹ 「合併」須改為與 §2.3.1 一致的說法。
*   **D46 的後果清單漏列 §2.3** ⟹ **可重用的教訓：裁定的後果清單是「它讓哪些句子變假」，
    而那要用 grep 找，不能靠記得。**
