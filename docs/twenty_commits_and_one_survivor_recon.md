...
## A. 驗收方對這份偵察的回應（2026-09-06）

### A.1 承重結論已複驗，而且比報告寫的更硬

〔讀〕`injections::clear(base)` **不接受任何集合**——它在**呼叫當下**列出整個目錄
（`for p in paths(base)?`）逐檔 `remove_file`，然後 `remove_dir`。
而 `load_all` 在 `universe.rs:1000`、`clear` 在 **`:1197`**，中間隔著近兩百行工作
（投影、求值、`put_root`、`put_commit`、`set_head`、`record_commit`）。

⟹ **「窗口是結構的，不是跑很多次總會遇到」這句話成立，而且不需要 `strace` 才看得出來。**
本份偵察的最小構造是**好的加分**，但結論在讀完那兩行之後就已經確定。

### A.2 三個「工單哪裡問錯了」——兩個成立，一個不成立

*   **第 1 點成立，而且是這份偵察最有價值的一句。** 驗收方的 A／B 兩分法切在
    「訊息 vs HEAD 拓撲」，**漏掉 `clear` 的範圍那一軸**，而值遺失的主詞正是它。
    **已據此把卡片改寫成三軸**（`WORK_QUEUE` §3），並記下：
    **軸 C 同樣不需要裁定**——`SPEC_10` §3 第 2 款＋D48 已蘊含
    「一次提交只得消費它自己 fold 過的成員」。
*   **第 2 點成立。** 「是不是 Q-016a／Q-040 造成的」確實容易被讀成歸咎；
    正確的歸屬是**它們只是停止吞掉一個原本就在的競態**。卡片已照此寫。
*   **⚠ 第 3 點不成立，就地更正。** 該點寫「Q4 先禁止清單又點了四個名字……
    **漏掉的是鑄造端 `record_commit` 的 `tips.next()`**」。
    **brief 的 Q4 逐字寫過它**：「`savepoint.rs::record_commit` 的 `ancestor: Option<&str>`
    是**單數**，covering parent 也被硬取一個（`vec![p]` 或 `tips.into_iter().next()`）」。
    它與那四個名字同段，且同樣標為「不完整，不得當成清單用」。
    **不影響 Q4 的答案**——那個不變式寫得對，鑄造端的對偶也指認得對。

### A.3 第 4 點（數字跳動）採納，並轉成一條約束

「約半數失敗」在複驗裡是 7／10／13 成功／20，驗收方自己量到的是 20／17／3。
**⟹ 那個比例不得寫進任何完成條件。** 不變的三件是：`oo log` **＝ 2**、
注入收尾 **＝ 0**、**值可以丟**。工單只能釘這三件。

### A.4 三軸的可開工性，據本偵察重排

| 軸 | 需要裁定？ | 依據 |
| :-- | :-- | :-- |
| **A 失敗訊息** | **否** | `SPEC_10` §2.2.1（不得洩漏實作表示／須指出座標與錯誤碼／各面一致） |
| **C `clear` 的範圍** | **否** | `SPEC_10` §3 第 2 款＋D48（工作集是集合，提交消費的是它 fold 的那一個） |
| **B HEAD 拓撲** | **是** | §4.1.1 寫於 D50 之前；甲／乙／丙 |

**⟹ A 與 C 今天就能開工單，且 C 會順手關掉 A 的一條 ENOENT 路徑**
（`clear` 不再 `remove_dir` ⟹ `read_dir` 那一支消失）。
**B 仍待用戶裁定，而本偵察已把丙的隱藏成本量成數字**：丙只有在 C 同時做掉時才便宜。
