...
## 9. 驗收紀錄(驗收方,2026-08-08)

**裁定:通過,零代修。** 交付 `ef4c2b7`。

### 9.1 探針完整性——機械證明

```
把 df172a6 的探針刪掉 4 行 #[ignore] → rustfmt → 與交付版 diff ⟹ IDENTICAL
```
...
| # | 量測 | 結果 |
| :-- | :--- | :--- |
| 1 | **diff 純度** | `universe.rs`、`main.rs`、探針(僅 ignore)、本文 §8。無範圍外檔案 |
| 2 | **探針** | **9/9** |
| 3 | **獨立重跑(整個 workspace)** | **1781 / 0 / 3**,183 區塊。算術:1777+4=1781、7−4=3 ✓ |
| 4 | **重複穩定** | 連跑五次,五次全同 |
| 5 | **對抗** | 見 §9.3——**逼出了工單 §6.4 預告的那個裁定** |
| 6 | **跨版本** | **實跑**:v0.11.1 二進位建立的 30 提交倉,交付版讀之 —— `log` 30 筆、`gc` 60/60/0 不變,衝突訊息為新形 |
...
```
前  Error: Evolution Conflict in "u.n": Conflict at
      Path(Path { anchor: Bare, segments: ["app"], span: Span { start: 0, end: 3 } })
後  Error: Evolution Conflict in "u.n": #conflict at app.db.opts.retries
REPL 後  Evolution Conflict: #conflict at app.db.opts.retries
```

**交付避開了我在裁定 R3 裡踩過的那個坑**:`universe.rs:435` 那條路徑的 `unify` 跑在
**欄位值**上,故 `path` 是相對的、需要絕對化;而其餘路徑跑在整個 staged 上、已是絕對。
交付以 `p == *c || p.starts_with(&format!("{c}."))` 分流,**不會印出 `app.app.db…`**,
並把理由寫進註解。**這正是我第一版裁定寫錯、被實測推翻的那一格。**

### 9.3 對抗:工單 §6.4 預告的裁定被逼出來了,但責任不在交付

| 輸入 | 印出的座標 |
| :--- | :--- |
| `cfg: { "my key": 1 }` 相衝 | `#conflict at cfg.my key` |
| `cfg: { "a.b": 1 }` 相衝 | `#conflict at cfg.a.b` |
| `cfg: { a: { b: 1 } }` 相衝 | `#conflict at cfg.a.b` ← **與上一列不可分** |
| `cfg: { 0: 1 }` 相衝 | `#conflict at cfg.0` ✓ 可導航 |

兩個**結構不同**的工作區給出**同一個座標字串**。但追下去之後,責任不在交付:

```
cfg."my key"  → Error:  --> 1:5      ← n/ 路徑語法拼不出來
cfg."a.b"     → Error:  --> 1:5      ← 同上
cfg.a.b       → _                    ← 靜默回 Top,而值其實是 2
cfg.0         → 3                    ← 數字鍵可導航
```

> **n/ 的路徑語法根本沒有「含點或含空白之鍵」的拼法。** 引號鍵**寫得進去**
> (`"a.b": 1` 合法),**讀不回來**。交付印的是唯一可印的東西。

而 `cfg.a.b` **靜默回 `_`** 而不是報錯——**照著印出來的座標走,會安靜地拿到錯的答案。**

**⟹ 零代修。** 缺陷在語言,不在本弧。

### 9.4 掛帳

1. **路徑語法沒有引號鍵的拼法**,而鍵可以是引號的。寫面與讀面不對稱。
   **這是語言層的缺口,不是引擎缺陷**,需要 SYNTAX 側的裁定。
2. **導航到不存在的巢狀路徑靜默回 `_`**,使「照座標走」變成不可靠的動作。
   與 1 合起來才是完整的危害。
3. **值的顯示印表機不加引號**(`--observe cfg` 印 `a.b: 2`),而 **`oo fmt` 會加**
   (`"a.b": 2`,且**實測 round-trip 成功**)。⟹ **fmt v2 沒有被違反**,
   但兩個印表機對同一個鍵給出不同的字。
4. `message` 裡仍是 Rust Debug(`Atom(Int(1), EffectTag(0), None)`)——工單 §3.2 已列為
   加分不列要求,交付未做,如實記。

### 9.5 規格側(驗收方待辦)

依 D39,本弧要在 SPEC_10 evolve 側**新增條文**(evolve 邊界撞 ⊥ 必須說出座標),
故切版走 **minor**。條文於收尾時寫,不在交付範圍。
