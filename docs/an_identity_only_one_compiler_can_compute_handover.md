...
## 8. 驗收（驗收方填，2026-09-19）

**結果：受理，零修補回合。**

### 8.1 純度與探針

R-0 動 `bn_serial.rs`（+354 行）、`value.rs`、**另一弧的探針 `snapshot_not_a_reading_probe_test.rs`**、本文件。
**本弧探針未動**（條件式檢查，不是無條件 echo——驗收方第一次的檢查寫成了無條件 echo，已改）。

**動別弧探針是正當的，而驗收方自己驗了那兩個數字**：
p5 釘的是帶 morphism 的宇宙根，而本弧授權移動任何含 `Code` 的位址。
〔量，兩個真二進位各自算一次〕`v0.53.0` → **`e59ea11d…`**（＝舊釘，故舊釘當時是對的）；
修補後 → **`43f838bf…`**（＝新釘）。
〔對照〕`x: 0` 根 `31745ef0…` **兩版相同** ⟹ **紀元＝且僅＝含 `Code` 的宇宙**。
且該探針原本的 docstring 就寫著「這個宇宙裡有沒有一個值，其身分編碼被本弧動過——`#blur` 與 `Value::Code`」
⟹ **它預告了這次移動**。

### 8.2 交付物 S1：表是完整的

〔量〕`ExprKind` **36 個變體**，交付給了 **36 個 `EX_` 標記**（`0x40`–`0x63`，剛好 36 格）
⟹ **窮舉，且沒有一列寫「同 Debug」**。含 Field 鍵（`Named`／`Quoted`／`Pattern`／`Path`）
與 Relation 的子編碼、`Prefix` 七個值、`Unary` 兩個值。**已寫入 `REAL_03` §6.2。**

### 8.3 量測（驗收方自量）

known-answer `~%Math./add (1,2)` → `3`（對照 `(1,3)` → `4`）。

**抽驗交付的修後位址表，五格全中**：`x -> add(x,x)` `abffa9b1…`／`x -> add(x,1)` `8a2122c0…`／
`x -> "hello"` `e65435df…`／`x -> """hello"""` `7c4b7d97…`／`y -> add(y,y)` `1b25ffb0…`。

**D71 在 `Code` 裡仍成立，而且比工單問的更深**：
〔量〕`x -> { a: 1, b: 2 }` `b4f72249…` ≠ `x -> { b: 2, a: 1 }` `48fbff37…`
⟹ **`Code` 裡的 combo 欄序是語法**；〔對照〕`x -> 1` 與 `x -> (1)` **同址** ⟹ 括號不是節點。
**已在規格裡寫明它與 §6.7 第四條（序必須是正準的）為何不衝突**：
在值裡鍵序排序，在 `Code` 裡源碼序**就是**「由內容決定」。

**全樹 ×3**（逐 target 聚合、保留失敗測試名）：

| 輪 | targets | passed | failed | 失敗測試名 | `^error` | cargo exit |
| :-- | --: | --: | --: | :-- | --: | --: |
| 1 | 235 | 2237 | **1** | `pin_concurrent_first_mint_yields_one_key` | 2 | **101** |
| 2 | 235 | 2238 | 0 | — | 0 | 0 |
| 3 | 235 | 2238 | 0 | — | 0 | 0 |

**先證明量測碰到了目標**：本弧探針 target 逐字 `4 passed; 0 failed`；`p5` 那支綠。
conformance **162／162**（rc=0，驗收方自跑）。

### 8.4 ⚠ run1 那一紅：驗收方第一次獨立重現了一列掛了一個月的 Inbox

失敗的是 `pin_concurrent_first_mint_yields_one_key`——**交付誠實報過**。
它對應 Inbox 那一列「**並行鑄造操作者身分會寫出半截 PKCS#8**」（`interrupt-candidate`），
而**那一列逐字寫著「⚠ 驗收方四次全跑一次都沒有重現」、「2026-08-31 驗收方嘗試重現，失敗」**
⟹ **本次 run1 是驗收方的第一次獨立重現。**

**不是本弧造成的**，三條理由：
(a) 同一個二進位三輪 **1 紅 2 綠** ⟹ 非決定性；一個決定性回歸會是 3／3。
(b) 本弧只動身分編碼（`bn_serial`／`value.rs` 的雜湊臂），而該支測的是
**並行第一次鑄金鑰時把 PKCS#8 寫了一半** ⟹ 檔案寫入，不是雜湊。
(c) 〔量〕在 `v0.53.0`（修補前）**隔離跑該支 6／6 綠** ⟹ 它只在全樹平行負載下出現，
與那一列既有的判定同形。**⚠ 而驗收方第一次量它時用錯了 target 名（`identity_persistence`
而非 `identity_persistence_probe_test`），六次 rc=101 且 `test result:` 為空——那是空讀數，已重做。**

⟹ **該 Inbox 列已據此更新**（第一次獨立重現、頻率 1／3、失敗形與舊敘述一致）。

### 8.5 交付指正了驗收方一處，而它是對的

工單 §2 與偵察 §3 寫著「〔引擎註解，**L2-65 已量**〕`to_nlang` 在約 10³ 層爆掉 64 MiB CLI 棧
⟹ 改用它會在語料裡現成的向量上爆掉」。**後半句是假的**，而「已量」兩字
**是驗收方把一句程式註解當成量測抄上去的**。

〔驗收方自量，`v0.53.0`，離開碼未經管線〕**3999 層（＝L2-65 的深度）三條路都 rc=0**：
`--observe f`（`to_nlang`，輸出 16,108 B）／`identify_and_store`（`Debug`）／`oo fmt`。
而 **20,000 與 80,000 層時剖析器先以具名邊界擋下**（`Parse Error … #stack_overflow`，rc=1，
**不是宿主崩潰**）⟹ **在今天經由剖析器可達的深度區間裡，`to_nlang` 從來輪不到爆。**

**I3 本身仍成立，理由已換成**：身分編碼不得從一個 pretty-printer 繼承深度上限，
而**剖析器的界不是對序列化器的保證**（語法樹也能從倉讀回來或由程式建起來）。
工單與偵察兩處已就地更正並留下原句。

### 8.6 規格結案（已落地）

`REAL_03` §6.2：值表新增 **`0x20` Code**、標明 `0x10` **不再承 Code**、
`0x21` 與 `0x40`–`0x63` **不是值標記**；原「`Code` 沒有規範性酬載」的按語改寫為已有，
並新增**完整的 Expr 節點表（36 列）**、Field 鍵與 Relation 的子編碼。
另新增兩段：§6.7 第四條與 `Code` 欄序為何不衝突；`Thunk`（`0x17`）是同族的另一半，**另開一弧**。

### 8.7 掛帳

*   **`Thunk`（`0x17`）**：酬載第一項仍是 `expr.to_nlang(0)` ⟹ 第二個實作若沒有一份
    被規範的 pretty-printer，一樣重算不出 Thunk 位址。交付指名、驗收方同意劃在本弧外。
    **改它會動 `0x17` 的既有位址** ⟹ 須另開一弧。
*   **O88** 未裁：`Code` 內的 `Float`／`Complex` 原子沿用值表的定點編碼，**有損**，同一個問題。
*   **α-等價**（`x -> add(x,x)` 對 `y -> add(y,y)` 仍異址）未裁，Inbox 另列。
