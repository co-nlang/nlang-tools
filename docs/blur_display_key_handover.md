...
**PASS——零代修(第二十二例)**。交付 commit `7686649`。

- **Diff 純度** ✓:單臂 23 行(族階 4 獨立臂,鍵=(cause.as_str,
  fuel, strategy 序號),全等 Equal;`as_str` 既有非新增,開單
  commit 驗證);探針檔僅 2 個 `#[ignore]` 移除。
- **獨立重跑** ✓:探針 7/7、workspace **1192/0/3**、conformance
  **116/116**、語料非 pending 74/0(67+7);display_order 17/17
  保綠。
- **對抗(病灶面直測)** ✓:雙 blur 聯集 CLI 8 次,caid 相對序
  6 升 2 降=鹽已非排序鍵(修前必 8/8 升序=caid 排序);混族
  `big | 2 | _` → `2 | #blur… | _` 族階正確;%caid 照印、身分
  鹽照舊(md5 跨行程異=%caid 值本身,設計內)。
