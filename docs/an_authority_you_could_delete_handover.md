...
known-answer ＋ 對照（新舊二進位皆過，離開碼直接取得）／**探針檔一字未動**（`git diff` 為空）／
conformance **162／162**／身分紅線 `31745ef0…`、**3 物件**、`7038e250…`／
全樹 ×3（**`--release`**，預設執行緒）**224／2146／0，無失敗測試名**。

**跨版本矩陣，真 `v0.43.0` 二進位，逐格自量**：

| | 結果 |
| :-- | :-- |
| 新倉 | `layout=5` |
| `layout=4` ＋ 新引擎普通 evolve | 寫出**無** `effect_tags` 的舊框；真 `v0.43.0` `status` rc=0、`commit` rc=0 |
| `layout=4` ＋ 新引擎 discharged evolve | **rc=1、注入 0、sidecar 不存在**，訊息指名 `migrate` |
| `v0.43.0` 開 `layout=5` | rc=1「`layout=5` is not supported; refusing to open」，**不是 corrupt** |
| `layout=4` ＋ `--pin` | rc=0，寫舊 pin 框，真 `v0.43.0` 讀得到 |
| **`layout=4` ＋ `--pin` 與 discharge 同時** | **rc=1、注入 0、不寫 sidecar** |

**最後一格是本輪最重要的自量**：交付方把 pin 的寫入閘由 `layout_declaration_is_current`
改成 `layout_writes_pin_frame`（越界一項，已自報並說明），**而那個改動開出一個邊緣**
——pin 在 `layout=4` 允許、discharge 不允許，兩者同時出現時若 pin 先寫成員而 discharge
悄悄退回 sidecar，本弧要拿掉的洞就會被寫回去。**實測沒有**：整個 evolve 在鑄任何成員
之前就被拒。**該越界成立。**

### A.2 ⚠ 一項不通過，而它打穿本弧自己的主張

〔量 2026-09-06，交付二進位，離開碼不經管線〕同一個成員，只改 `effect_tags` 一行：

```
effect_tags: 1              → commit rc=1   閘開火                    ✓
effect_tags: not-a-number   → commit rc=1   effect_tags unreadable    ✓（S2 已修）
effect_tags: 9   (io|未知)   → commit rc=1   io 位元存活遮蔽後仍在      ✓
effect_tags: 8   (只有未知位元) → commit rc=0   Commit successful        ✗
```

〔讀〕`value.rs:78` `from_bits` 對未知位元**遮蔽**，其註解逐字寫著理由——
「**unknown bits must not become capabilities**」。**那個理由對它自己是對的**：
問句若是「這能不能授予什麼」，未知位元當然不得成為能力。
**但這裡的問句是「有沒有東西被 discharge」**，而遮蔽把一組未知位元變成空集合，
空集合讀起來就是「沒有 discharge」。

⟹ **這是同一個「讀不出來被當成沒有」，只是下沉了一層到表示層**——
而那正是 S2 明文要關的門。交付方為**壞 JSON** 關了它，**未為未知位元關**。

**⚠ 而它否定了本弧宣稱買到的東西。** 射程逐字寫著「移除授權事實**必須改寫承載值本身
的那個物件**」。`effect_tags: 8` 確實改寫了那個物件——**但值一個位元組都沒動**，
只有授權事實蒸發了。**「不再可分離」在這一格不成立。**

**⚠ 這是驗收方的射程漏的，不是交付方沒做工單**：R2 我寫的是「**刪掉**格子」，
而不變式是「**移除**授權事實」——**刪是一種方式，改是另一種**，而我只釘了前一種。
**這是同一個病的第五次**（`feedback_scope_invariants_not_mechanisms`）。

### A.3 修補回合 1 的射程（一項）

**B1 — 讀得出來但本引擎不完全認得的標籤集，不得被當成「沒有 discharge」。**

*   **不變式（不是機制）**：**任何無法被本引擎完整表示的持久化標籤集，必須拒絕，
    而不得降級為較小的集合。**
*   **⚠ 不得改 `from_bits` 的語義**：它的遮蔽對它的其他呼叫者是對的，
    且它的註解已經說出理由。**要驗證的是解碼路徑**，不是那個共用函式。
*   **不得放寬** `unknown metadata` 那條既有的嚴格性。
*   **拒絕必須誠實**：說出是「這個標籤集本引擎讀不完整」，
    **不得**報成完整性失效（`REAL_03` §6.8）、**不得**報成「沒有 discharge」。

### A.4 探針（驗收方已加一支，**基線紅**）

**R5** `a_tag_set_this_engine_cannot_fully_read_is_never_no_discharge`
——同一個成員只改 `effect_tags` 一行為 `8`，斷言 **`rc != 0`**。

**斷言寫成「拒絕」而非「某個特定錯誤」是刻意的**，因為它**兩個未來都對**：
若位元 8 永遠不是標籤，它必須以「讀不完整」被拒；
若日後某個引擎定義了它，那就是一次 discharge，必須因未出示能力而被拒。
**兩條路都不是 rc=0。**

### A.5 交付方做對而值得記的

*   **越界只有一項，自報、逐行說明、且理由成立**（見 A.1 最後一格的實測）。
*   **S2 的兩個出口都測了**（壞 JSON ／多餘欄位），且**明文未放寬** `bail`。
*   **N.3 指出工單兩句會打架**（「普通 discharge 仍須寫舊形式」對「需要新欄位者須先拒絕」），
    並說明取捨——**那是對的**：在 `layout=4` 再寫 `.oo/effect_pending`
    等於把本弧要拿掉的格子寫回去。**工單那句是驗收方寫壞的，此處更正。**
*   **誠實紀錄了 WSL 中斷造成的 29 個截斷二進位並重編後才計數。**

### A.6 一件與產品無關但值得記的

交付方的全樹是 **debug ＋ `--test-threads=1`**，一輪約 20 分鐘。
驗收方同一棵樹以**工單所寫的 `--release`**、預設執行緒跑，一輪 **3 分 14 秒**。
⟹ **那 20 分鐘是呼叫方式的產物，不是產品變慢**；工單逐字寫的是 `--release`。
