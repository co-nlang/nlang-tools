...
**判定:通過——零代修(第六例)。**
(過程注記:宿主機於交付中途崩潰重啟;驗收方重啟後於未提交工作樹上
先行全套量測,與執行者恢復後補交之 `fef564c` 內容一致——量測即為
tag 內容之實測。)

獨立重測:探針 diff 僅 6 個 `#[ignore]` 移除、斷言原封;探針 13/13;
workspace **813/0/3**(800+13,吻合);語料 **72/0**;conformance
**52/52**(L1-32 關門)。

diff 逐條對裁定:單段逐支投影(尾段外層迴圈續走,無雙重投影——
陷阱 3 照做)、⊥ 支剔除、空 → #invalid_path、normalize_union、
Top 缺欄支保留、效果取支間 max、沿用同一 ctx。鄰行尾空白清理(良性)。

對抗性邊界戳刺(工單外,全數健康):
- 支內欄位本身是 Union:`({a:(1|2)} | {a:3}).a` → `1 | 2 | 3`(攤平)。
- 導航結果續管道:`(….a) |> inc` → `2 | 3`。
- 混深多段:`({x:{y:1}} | {x:2}).x.y` → `1`(第二段 atom 支 ⊥ 正確剔除
  ——逐段匯總語義實測)。
- 巢狀 union 字面量構造期已攤平,導航照常。
